...
RUNNER = REPO_ROOT / "automation/orchestration/self_audit/Get-AiOsSelfDevelopmentPacketRouter.DRY_RUN.ps1"
...
def test_runner_emits_json_only_with_output_json() -> None:
    ...
    assert "AIOS Self-Development Packet Router" not in raw
    assert parsed["schema"] == "AIOS_SELF_DEVELOPMENT_PACKET_ROUTER_RESULT.v1"
    assert parsed["recommended_packet"]["packet_id"] == "AIOS-VALIDATOR-EVIDENCE-ROUTER-DRYRUN-V1"
...
def test_runner_passes_expected_branch_to_self_audit() -> None:
    text = RUNNER.read_text(encoding="utf-8")

    assert "automation/orchestration/self_audit/Invoke-AiOsSelfAuditLoop.DRY_RUN.ps1" in text
    assert "-ExpectedBranch $ExpectedBranchValue" in text
...
def test_runner_refuses_dirty_worktree_when_fail_on_dirty_is_true(tmp_path: Path) -> None:
    ...
    assert "CONFIDENCE_INSUFFICIENT" in parsed["stop_conditions"]
    assert parsed["repo_state"]["dirty_allowed_for_router_validation"] is False
...
def test_runner_does_not_call_excluded_write_capable_scripts() -> None:
    text = RUNNER.read_text(encoding="utf-8")
    surface_block = text.split("$surfaces = ", 1)[1].split("$surfaceResults = ", 1)[0]
    excluded = [
        "automation/orchestration/packet_generator/New-AiOsCodexPacket.DRY_RUN.ps1",
        "automation/orchestration/control/Get-AiOsCodexPacketDraftPreview.DRY_RUN.ps1",
...
def test_runner_no_write_proof_contains_forbidden_delta_detection() -> None:
    ...
    assert "BLOCKED_BY_WRITE_SURFACE_RISK" not in text
    assert "Compare-NoWriteState" in text


def test_self_audit_runner_allows_exact_router_files_without_broad_directory() -> None:
