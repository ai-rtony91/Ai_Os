...
def test_forex_continuation_recommender_reports_known_sprints_and_recommendation():
    ...
    assert res["latest_forex_sprint_detected"] == "SPRINT_15"
    assert res["recommended_next_packet_id"] == "AIOS-FOREX-PAPER-RISK-DECISION-ROUTER-APPLY-V1"
    assert (
        res["recommended_next_packet_title"]
        == "feat(forex): add paper risk decision router"
    )
    assert res["recommended_lane"] == "PAPER_RISK_DECISION_ROUTER"
