...
def test_queue_skips_completed_one_action_local_apply_executor_and_selects_result_collector(tmp_path):
    module = load_module()
    completed_paths = (
        module.CORE_STATUS_READER_PATHS
        + module.RUN_SUMMARY_VIEW_PATHS
        ...
        + module.ONE_ACTION_APPLY_RUNNER_PATHS
        + module.ONE_ACTION_EXECUTE_GATE_PATHS
        + module.ONE_ACTION_LOCAL_APPLY_EXECUTOR_PATHS
    )
    for relative_path in completed_paths:
        target = tmp_path / relative_path
        target.parent.mkdir(parents=True, exist_ok=True)
    ...
    assert items_by_action["build_self_build_one_action_local_apply_executor"]["status"] == "completed"
    assert items_by_action["build_self_build_one_action_execution_result_collector"]["status"] == "ready"
    assert selected["selected_next_action"] == "build_self_build_one_action_execution_result_collector"
    assert selected["selected_queue_item"]["allowed_paths"] == module.ONE_ACTION_EXECUTION_RESULT_COLLECTOR_PATHS
    assert selected["selected_queue_item"]["validators"] == module.ONE_ACTION_EXECUTION_RESULT_COLLECTOR_VALIDATORS
    assert all(value is False for value in selected["selected_queue_item"]["protected_action_flags"].values())
