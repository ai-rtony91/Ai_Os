...
from automation.forex_engine.oanda_demo_owner_run_sanitized_telemetry_exercise_v1 import (  # noqa: E402
    ...
    classify_sanitized_telemetry_exercise,
...
from scripts.forex_delivery.run_oanda_demo_owner_run_sanitized_telemetry_exercise_v1 import (  # noqa: E402
    main as script_main,
)
...
def capture_result(
    capture_status: str = OWNER_RUN_SANITIZED_TELEMETRY_CAPTURE_ACCEPTED,
    adapter_status: str = SANITIZED_OWNER_RUN_TELEMETRY_ACCEPTED,
    *,
    ready: bool = True,
) -> dict:
    result = {
        ...
        "sanitized_broker_telemetry_ready": ready,
        ...
        "profit_evidence": False,
        "blockers": [],
        "next_action": "FEED_SANITIZED_TELEMETRY_TO_TRADE_320_PL_REFRESH",
    }
    if capture_status != OWNER_RUN_SANITIZED_TELEMETRY_CAPTURE_ACCEPTED:
        result["sanitized_broker_telemetry_ready"] = False
    return result
...
def test_default_result_does_not_call_broker_and_reports_not_requested() -> None:
    ...
    assert result["exercise_status"] == OWNER_RUN_SANITIZED_TELEMETRY_EXERCISE_NOT_REQUESTED
...
def test_default_capture_status_remains_not_requested() -> None:
    result = run_owner_run_sanitized_telemetry_exercise()
...
def test_valid_sanitized_accepted_capture_maps_to_exercise_accepted() -> None:
    result = run_owner_run_sanitized_telemetry_exercise(
        owner_run_read_broker_now=True,
        evidence=valid_sanitized_trade_320_evidence(),
    )

    assert result["exercise_status"] == OWNER_RUN_SANITIZED_TELEMETRY_EXERCISE_ACCEPTED
    assert result["adapter_status"] == SANITIZED_OWNER_RUN_TELEMETRY_ACCEPTED
    assert result["sanitized_evidence_persisted"] is True
...
def test_no_secrets_written_true_does_not_reject_as_secret_risk() -> None:
    evidence = valid_sanitized_trade_320_evidence()
...
def test_secrets_written_false_does_not_reject_as_secret_risk() -> None:
    evidence = valid_sanitized_trade_320_evidence()
    evidence["secrets_written"] = False
...
def test_secrets_written_true_rejects_as_secret_risk() -> None:
    evidence = valid_sanitized_trade_320_evidence()
    evidence["secrets_written"] = True
    ...
    assert (
        result["exercise_status"]
        == OWNER_RUN_SANITIZED_TELEMETRY_EXERCISE_REJECTED_SECRET_RISK
    )
    assert result["sanitized_broker_telemetry_ready"] is False


def test_raw_broker_payload_persisted_false_does_not_reject_as_raw_payload() -> None:
    evidence = valid_sanitized_trade_320_evidence()
    evidence["raw_broker_payload_persisted"] = False
...
def test_raw_broker_payload_persisted_true_rejects_as_raw_payload() -> None:
    evidence = valid_sanitized_trade_320_evidence()
    evidence["raw_broker_payload_persisted"] = True
...
def test_account_id_read_performed_false_does_not_reject_as_secret_risk() -> None:
    evidence = valid_sanitized_trade_320_evidence()
...
def test_account_id_read_performed_true_rejects_as_secret_risk() -> None:
    evidence = valid_sanitized_trade_320_evidence()
    evidence["account_id_read_performed"] = True
...
def test_order_placement_performed_false_does_not_reject() -> None:
    evidence = valid_sanitized_trade_320_evidence()
    evidence["order_placement_performed"] = False
...
def test_order_placement_performed_true_rejects() -> None:
    evidence = valid_sanitized_trade_320_evidence()
    ...
    assert result["exercise_status"] == OWNER_RUN_SANITIZED_TELEMETRY_EXERCISE_INVALID_SHAPE
    assert result["sanitized_broker_telemetry_ready"] is False


def test_broker_blocked_capture_maps_to_exercise_broker_blocked() -> None:
    evidence = valid_sanitized_trade_320_evidence()
    ...
    assert result["exercise_status"] == OWNER_RUN_SANITIZED_TELEMETRY_EXERCISE_BROKER_BLOCKED
    assert result["sanitized_broker_telemetry_ready"] is False


def test_helper_missing_capture_maps_to_exercise_helper_missing() -> None:
    result = classify_sanitized_telemetry_exercise(
        capture_result(OWNER_RUN_SANITIZED_TELEMETRY_CAPTURE_HELPER_MISSING)
    )

    assert result["exercise_status"] == OWNER_RUN_SANITIZED_TELEMETRY_EXERCISE_HELPER_MISSING


def test_secret_risk_maps_to_exercise_rejected_secret_risk() -> None:
    result = classify_sanitized_telemetry_exercise(
        capture_result(OWNER_RUN_SANITIZED_TELEMETRY_CAPTURE_REJECTED_SECRET_RISK)
    )
...
def test_raw_payload_maps_to_exercise_rejected_raw_payload() -> None:
    result = classify_sanitized_telemetry_exercise(
        capture_result(OWNER_RUN_SANITIZED_TELEMETRY_CAPTURE_REJECTED_RAW_PAYLOAD)
    )
...
def test_invalid_shape_maps_to_exercise_invalid_shape() -> None:
    result = classify_sanitized_telemetry_exercise(
        capture_result(OWNER_RUN_SANITIZED_TELEMETRY_CAPTURE_INVALID_SHAPE)
    )
...
def test_fake_and_mock_dashboard_numbers_remain_forbidden() -> None:
    result = classify_sanitized_telemetry_exercise(capture_result())
...
def test_cli_default_emits_json_without_broker_call() -> None:
    ...
    assert code == 0
    assert payload["script_status"] == OWNER_RUN_SANITIZED_TELEMETRY_EXERCISE_NOT_REQUESTED
...
def test_report_writer_includes_safety_statements_and_raw_payload_false(
    ...
    result = run_owner_run_sanitized_telemetry_exercise()
    report_path = tmp_path / "telemetry_exercise_report.md"

    write_owner_run_sanitized_telemetry_exercise_report(result, report_path)
