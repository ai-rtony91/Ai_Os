...
from automation.forex_engine.supervised_demo_owner_approval_epic_v1 import (
    SUPERVISED_DEMO_OWNER_APPROVAL_BLOCKED,
    ...
    supervised_demo_owner_approval_epic_to_markdown,
    supervised_demo_owner_approval_epic_to_operator_text,
)
from scripts.forex_delivery import run_demo_owner_approval_phrase_gate_v1 as phrase_runner
from scripts.forex_delivery import run_supervised_demo_owner_approval_epic_v1 as epic_runner
from scripts.forex_delivery import run_supervised_demo_owner_approval_packet_v1 as packet_runner
...
def test_epic_ready_sample_passes() -> None:
    result = run_supervised_demo_owner_approval_epic(
        build_sample_supervised_demo_owner_approval_ready_input()
    )

    assert result.classification == SUPERVISED_DEMO_OWNER_APPROVAL_READY_FOR_OWNER_REVIEW
...
def test_epic_blocked_sample_blocks() -> None:
    ...
    assert result.classification == SUPERVISED_DEMO_OWNER_APPROVAL_BLOCKED
...
def test_epic_operator_answer_is_plain_english() -> None:
    text = supervised_demo_owner_approval_epic_to_operator_text(run_supervised_demo_owner_approval_epic())

    assert "Supervised demo owner approval status" in text
...
def test_epic_markdown_has_title() -> None:
    markdown = supervised_demo_owner_approval_epic_to_markdown(run_supervised_demo_owner_approval_epic())

    assert markdown.startswith("# Supervised Demo Owner Approval Epic V1")


def test_epic_json_has_required_keys() -> None:
    payload = supervised_demo_owner_approval_epic_to_jsonable_dict(run_supervised_demo_owner_approval_epic())
...
def test_ready_output_deterministic() -> None:
    first = supervised_demo_owner_approval_epic_to_jsonable_dict(run_supervised_demo_owner_approval_epic())
    second = supervised_demo_owner_approval_epic_to_jsonable_dict(run_supervised_demo_owner_approval_epic())

    assert first == second


def test_blocked_output_deterministic() -> None:
    first = supervised_demo_owner_approval_epic_to_jsonable_dict(
        run_supervised_demo_owner_approval_epic(build_sample_supervised_demo_owner_approval_blocked_input())
    )
    second = supervised_demo_owner_approval_epic_to_jsonable_dict(
        run_supervised_demo_owner_approval_epic(build_sample_supervised_demo_owner_approval_blocked_input())
    )
...
@pytest.mark.parametrize("runner_module", (phrase_runner, packet_runner, epic_runner))
...
def test_manual_finalization_report_includes_exact_git_add_commands() -> None:
    text = (REPO_ROOT / "Reports/forex_delivery/AIOS_FOREX_SUPERVISED_DEMO_OWNER_APPROVAL_MANUAL_FINALIZATION_V1.md").read_text(
        encoding="utf-8"
    )
...
def test_epic_report_says_no_trade_placed() -> None:
    text = (REPO_ROOT / "Reports/forex_delivery/AIOS_FOREX_SUPERVISED_DEMO_OWNER_APPROVAL_EPIC_REPORT_V1.md").read_text(
        encoding="utf-8"
    )
...
def test_protected_permissions_stay_false_in_every_top_level_output() -> None:
    result = run_supervised_demo_owner_approval_epic()
    payload = supervised_demo_owner_approval_epic_to_jsonable_dict(result)
