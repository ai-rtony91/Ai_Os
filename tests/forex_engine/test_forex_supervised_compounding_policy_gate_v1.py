...
from automation.forex_engine import forex_supervised_compounding_policy_gate_v1 as gate
...
RUNNER = (
    ...
    / "scripts"
    / "forex_delivery"
    / "run_forex_supervised_compounding_policy_gate_v1.py"
)


def _result(builder=gate.build_sample_ready_input):
    return gate.evaluate_forex_supervised_compounding_policy_gate(builder())


@pytest.mark.parametrize(
    ("builder", "classification"),
    (
        (gate.build_sample_ready_input, gate.SUPERVISED_COMPOUNDING_POLICY_READY),
        (
            gate.build_sample_partial_input,
            gate.SUPERVISED_COMPOUNDING_REQUIRE_MORE_EVIDENCE,
        ),
        (
            gate.build_sample_unsafe_input,
            gate.SUPERVISED_COMPOUNDING_BLOCKED_UNSAFE,
        ),
        (
            gate.build_sample_schema_invalid_input,
            gate.SUPERVISED_COMPOUNDING_BLOCKED_SCHEMA_INVALID,
        ),
    ),
)
...
def test_no_vacation_mode_execution_is_authorized():
    ...
    assert result.supervised_compounding_gate_authorizes_vacation_mode is False
    assert "vacation_mode_execution" in result.blocked_actions


def test_compounding_policy_percent_is_calculated_correctly():
    ready = _result(gate.build_sample_ready_input)
    partial = _result(gate.build_sample_partial_input)
    assert ready.compounding_policy_percent == 100.0
    expected_partial = round(
        (partial.ready_surface_count / partial.total_surface_count) * 100,
        2,
    )
    assert partial.compounding_policy_percent == expected_partial
...
def test_missing_surfaces_are_reported():
    ...
    assert "source_evidence_quality_ready" in result.missing_surfaces
    assert "compounding_requires_news_filter" in result.missing_surfaces
...
def test_next_packet_preview_is_sos_owner_alert_bridge():
    assert _result().next_packet_preview == gate.NEXT_PACKET_PREVIEW
    assert _result().next_packet_preview == "AIOS-FOREX-SOS-OWNER-ALERT-BRIDGE-V1"


def test_next_safe_action_is_explicit():
    result = _result()
    assert "Review the supervised compounding policy result" in result.next_safe_action
    assert "do not approve broker access" in result.next_safe_action


def test_mapping_input_works():
    source = gate.build_sample_ready_input().source_statistical_profit_result
    result = gate.evaluate_forex_supervised_compounding_policy_gate(
        {
            "source_statistical_profit_result": source,
            "compounding_policy_surfaces": {
                name: True for name in gate.COMPOUNDING_POLICY_SURFACES
            },
            "owner_compounding_label": "pending_owner_review",
            "owner_notes_sanitized": "mapping input",
        }
    )
    assert result.classification == gate.SUPERVISED_COMPOUNDING_POLICY_READY


def test_dataclass_like_input_works():
    class AttributeInput:
        source_statistical_profit_result = (
            gate.build_sample_ready_input().source_statistical_profit_result
        )
        compounding_policy_surfaces = {
            name: True for name in gate.COMPOUNDING_POLICY_SURFACES
        }
    ...
    result = gate.evaluate_forex_supervised_compounding_policy_gate(AttributeInput())
    assert result.classification == gate.SUPERVISED_COMPOUNDING_POLICY_READY


def test_unsafe_sample_reports_unsafe_fragments_but_output_flags_stay_false():
    result = _result(gate.build_sample_unsafe_input)
    assert result.classification == gate.SUPERVISED_COMPOUNDING_BLOCKED_UNSAFE
    assert result.unsafe_fragments_detected
    assert result.protected_flags["bank_movement_allowed"] is False
    assert result.protected_flags["withdrawal_allowed"] is False
    assert result.protected_flags["deposit_allowed"] is False
...
def test_schema_invalid_sample_reports_schema_errors():
    ...
    assert result.classification == gate.SUPERVISED_COMPOUNDING_BLOCKED_SCHEMA_INVALID
    assert any(
        "source_statistical_profit_result" in item
        for item in result.unsafe_fragments_detected
    )
    assert any(
        "compounding_policy_surfaces" in item
        for item in result.unsafe_fragments_detected
    )
...
def test_json_serializable():
    ...
    assert "SUPERVISED_COMPOUNDING_POLICY_READY" in payload


def test_markdown_output():
    text = gate.to_markdown(_result())
    assert text.startswith("# AIOS Forex Supervised Compounding Policy Gate V1")
...
def test_cli_json_works():
    ...
    assert payload["classification"] == gate.SUPERVISED_COMPOUNDING_POLICY_READY
...
def test_cli_markdown_works():
    ...
    assert "Classification: `SUPERVISED_COMPOUNDING_POLICY_READY`" in completed.stdout
...
@pytest.mark.parametrize(
    ...
    (
        ...
        '"supervised_compounding_gate_authorizes_trading": true',
        '"supervised_compounding_gate_authorizes_execution": true',
        '"supervised_compounding_gate_authorizes_compounding": true',
        '"supervised_compounding_gate_authorizes_vacation_mode": true',
    ),
)
