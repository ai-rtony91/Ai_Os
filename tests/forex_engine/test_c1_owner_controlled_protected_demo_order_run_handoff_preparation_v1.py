...
from scripts.forex_delivery.run_c1_owner_controlled_protected_demo_order_run_handoff_preparation_v1 import (
    generate_artifacts,
)
...
def test_default_owner_input_none_blocks_owner_input_required() -> None:
    ...
    assert result["owner_run_handoff_status"] == "NOT_READY"
    assert result["post_p10_score"] == 100
    assert (
        result["next_required_lane"]
        == "P6B_OWNER_SUPPLY_SANITIZED_SNAPSHOT_AND_APPROVAL_INPUT"
    )
    assert result["owner_run_handoff_packet_created"] is False
...
def test_valid_sanitized_owner_input_prepares_for_p11() -> None:
    result = (
        evaluate_c1_owner_controlled_protected_demo_order_run_handoff_preparation(
            _valid_owner_input()
        )
    ...
    assert result["owner_run_handoff_status"] == "P11_READY"
    assert (
        result["next_required_lane"]
        == "P11_EXPLICIT_OWNER_APPROVED_PROTECTED_DEMO_ORDER_RUN_PACKET_REVIEW"
    )
    assert result["owner_run_handoff_packet_created"] is True
    assert result["owner_run_handoff_packet"]["handoff_packet_type"] == (
        "INERT_OWNER_RUN_PREPARATION_ONLY"
    )


def test_owner_run_handoff_packet_created_only_on_valid_path() -> None:
    valid_result = (
        evaluate_c1_owner_controlled_protected_demo_order_run_handoff_preparation(
            _valid_owner_input()
        )
    )
    bad_owner_input = _valid_owner_input()
    bad_owner_input["owner_decision"] = "REQUEST_CHANGES"
    ...
    assert valid_result["owner_run_handoff_packet_created"] is True
    assert invalid_result["owner_run_handoff_packet_created"] is False
...
def test_rejected_request_changes_or_invalid_owner_input_does_not_route_to_p11() -> None:
...
def test_missing_owner_run_handoff_review_marker_fails_p10() -> None:
    owner_input = _valid_owner_input()
    owner_input["owner_run_handoff_review"] = False
    ...
    assert result["p10_handoff_status"] == "P10_HANDOFF_FAILED_REPAIR_REQUIRED"


def test_missing_credential_handling_review_marker_fails_p10() -> None:
    owner_input = _valid_owner_input()
...
def test_missing_broker_connection_review_marker_fails_p10() -> None:
    owner_input = _valid_owner_input()
...
def test_generate_artifacts_default_outputs_exist() -> None:
    ...
    assert result["p10_handoff_status"] == "P10_HANDOFF_BLOCKED_OWNER_INPUT_REQUIRED"
    assert result["owner_run_handoff_status"] == "NOT_READY"
    assert (
        result["next_required_lane"]
        == "P6B_OWNER_SUPPLY_SANITIZED_SNAPSHOT_AND_APPROVAL_INPUT"
