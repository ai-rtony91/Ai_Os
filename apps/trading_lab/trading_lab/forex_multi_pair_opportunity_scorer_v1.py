"""Governed review-only multi-pair opportunity scorer for Forex."""
...
def score_multi_pair_opportunities(
    ...
    *,
    ...
    policy, warnings = _normalize_policy(risk_policy)
    context = dict(execution_context or {})
    context_blockers = _global_blockers(context)

    normalized: list[dict[str, Any]] = []
    rejection_reasons = list(warnings)
    ...
    total_portfolio_risk_percent = _round_float(sum(item["proposed_allocation_percent"] for item in ranked_eligible), 6)
    proposed_allocation_percent = total_portfolio_risk_percent

    if ranked_eligible and total_portfolio_risk_percent <= 0:
        safe_next_action = (
            "No allocation is possible under policy caps. Collect stronger evidence, repair spread/slippage, "
            "and rerun read-only scoring."
        )
    elif ranked_eligible:
        safe_next_action = (
            "Paper/simulation multi-pair allocation is ready for review. "
            "No live capital scaling or execution occurs until repeatability evidence is proved and owner gates are satisfied."
        )
    elif "input_must_be_sequence" in rejection_reasons or "no_valid_pair_metrics" in rejection_reasons:
        safe_next_action = "Provide valid pair metrics before rerunning read-only scoring."
    else:
        safe_next_action = "Repair rejected pair blockers and rerun read-only scoring."

    rejection_reasons = _unique(rejection_reasons + [reason for item in normalized for reason in item["rejection_reasons"]])
    ...
    return {
        ...
        "risk_policy": {
            "kill_switch_active": bool(context.get("kill_switch_active", False) or context.get("kill_switch_enabled", False)),
            "daily_loss_stop_reached": bool(
                context.get("daily_loss_stop_reached", context.get("daily_loss_limit_reached", False)),
            ),
            "margin_guard_ok": bool(context.get("margin_guard_ok", context.get("margin_guard_active", True))),
...
def _normalize_pair_payload(pair_payload: Mapping[str, Any], index: int, policy: Mapping[str, Any]) -> dict[str, Any]:
    pair = _normalize_pair(pair_payload.get("pair"), default=f"PAIR_{index:03d}")
    rejection_reasons: list[str] = []

    confidence_score = _extract_score(pair_payload, ("confidence_score", "confidence", "signal_strength", "confidence_pct"))
    spread_score = _resolve_spread_score(pair_payload, policy)
    volatility_score = _resolve_volatility_score(pair_payload, policy)
    liquidity_score = _extract_score(pair_payload, ("liquidity_score", "liquidity", "liquidity_quality_score"), default=60.0)
    expectancy_score = _extract_score(pair_payload, ("expectancy_score", "expectancy", "expectancy_ratio", "edge_score"), default=45.0)
    ...
    evidence_quality = _extract_score(pair_payload, ("evidence_quality_score", "evidence_score", "quality_score"), default=None)
    ...
    if _contains_forbidden_terms(sizing_model) or bool(pair_payload.get("is_martingale")) or bool(pair_payload.get("is_revenge")):
...
def _proposed_allocations(
    ...
    while remaining and remaining_budget > 0:
        ...
        if not capped:
            for pair, weight in remaining.items():
                allocation[pair] = _round_float(allocation[pair] + remaining_budget * (weight / total_weight), 6)
            break
...
def _correlation_factor(
    ...
    correlations = [_safe_float(source_row.get(other_pair)) for other_pair in other_pairs if _safe_float(source_row.get(other_pair)) is not None]
    if not correlations:
        return 1.0
...
def _global_blockers(execution_context: Mapping[str, Any]) -> list[str]:
    ...
    if bool(
        execution_context.get("daily_loss_stop_reached", execution_context.get("daily_loss_limit_reached", False)),
    ):
    ...
    if not bool(execution_context.get("margin_guard_ok", execution_context.get("margin_guard_active", True))):
...
def _extract_score(payload: Mapping[str, Any], keys: Sequence[str], *, default: float | None = None, max_score: float = 100.0) -> float:
    for key in keys:
        raw = payload.get(key)
...
def _normalize_policy(risk_policy: Mapping[str, Any] | None) -> tuple[dict[str, Any], list[str]]:
    ...
    policy = {
        ...
        "min_sample_size": _normalize_int(policy_input.get("min_sample_size"), "min_sample_size", DEFAULT_MIN_SAMPLE_SIZE, 1, 1_000_000, warnings),
...
def _normalize_percent(raw: Any, field_name: str, default: float, minimum: float, maximum: float, warnings: list[str]) -> float:
    value = _safe_float(raw)
    if value is None:
        if raw is not None:
...
def _normalize_int(raw: Any, field_name: str, default: int, minimum: int, maximum: int, warnings: list[str]) -> int:
    value = _safe_int(raw)
    if value is None:
        if raw is not None:
            warnings.append(f"{field_name}_invalid_or_missing_using_default")
