...
from automation.forex_engine.oanda_live_microtrade_profit_proof_evidence_depth_plan_v1 import (  # noqa: E402
    build_oanda_live_microtrade_profit_proof_evidence_depth_plan,
    build_sample_breakeven_depth_plan_input,
    build_sample_loss_depth_plan_input,
    build_sample_missing_owner_result_depth_plan_input,
    build_sample_profit_depth_plan_input,
    build_sample_unsafe_depth_plan_input,
    to_jsonable_dict,
    to_markdown,
    to_operator_text,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a local-only evidence-depth plan for a weak profit candidate."
    )
...
def _select_sample(args: argparse.Namespace):
    if args.sample_loss:
        return build_sample_loss_depth_plan_input()
    if args.sample_breakeven:
        return build_sample_breakeven_depth_plan_input()
    if args.sample_missing:
        return build_sample_missing_owner_result_depth_plan_input()
    if args.sample_unsafe:
        return build_sample_unsafe_depth_plan_input()
    return build_sample_profit_depth_plan_input()


def main() -> int:
    args = parse_args()
    result = build_oanda_live_microtrade_profit_proof_evidence_depth_plan(
        _select_sample(args)
    )
    if args.json:
