...
from automation.forex_engine.forex_vacation_mode_final_readiness_decision_v1 import (  # noqa: E402
    build_sample_partial_input,
    build_sample_ready_input,
    build_sample_schema_invalid_input,
    build_sample_unsafe_input,
    evaluate_forex_vacation_mode_final_readiness_decision,
    to_jsonable_dict,
    to_markdown,
    to_operator_text,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preview AIOS Forex Vacation Mode final readiness decision."
    )
...
def main() -> int:
    args = parse_args()
    result = evaluate_forex_vacation_mode_final_readiness_decision(_select_sample(args))
    if args.json:
