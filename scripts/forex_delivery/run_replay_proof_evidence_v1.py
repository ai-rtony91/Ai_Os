"""Run the local-only replay proof evidence adapter V1."""
...
def main(argv: list[str] | None = None, stdout: TextIO | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Replay Proof Evidence V1 sample.")
    ...
    sample = build_sample_replay_summary()
    if args.blocked:
        sample["mismatch_count"] = 1
    if args.incomplete:
        sample.pop("replay_id", None)
    result = evaluate_replay_proof_evidence(sample)
