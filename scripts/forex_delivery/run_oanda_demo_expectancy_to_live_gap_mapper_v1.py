...
from automation.forex_engine.oanda_demo_expectancy_to_live_gap_mapper_v1 import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preview OANDA demo expectancy to live evidence gap mapping."
    )
...
def _select_sample(args: argparse.Namespace):
    ...
    if args.sample_ready_gap_review:
        return build_sample_ready_gap_mapper_input()
...
def main() -> int:
    args = parse_args()
    result = map_oanda_demo_expectancy_to_live_evidence_gaps(_select_sample(args))
    if args.json:
        print(json.dumps(oanda_demo_expectancy_to_live_gap_mapper_to_jsonable_dict(result), indent=2))
    elif args.markdown:
        print(oanda_demo_expectancy_to_live_gap_mapper_to_markdown(result), end="")
    else:
        print(oanda_demo_expectancy_to_live_gap_mapper_to_operator_text(result))
    return 0
