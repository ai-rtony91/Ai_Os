"""CLI runner for local-only AIOS Forex Expectancy Strength Router V1."""
...
def main(argv: list[str] | None = None, stdout: TextIO | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run local-only AIOS Forex Expectancy Strength Router V1."
    )
    ...
    evidence = (
        ...
        else build_sample_mixed_expectancy_input()
    )
    result = route_expectancy_strength(evidence)
    out = stdout if stdout is not None else sys.stdout
    if args.json:
        json.dump(result_to_jsonable_dict(result), out, indent=2, sort_keys=True)
        out.write("\n")
    elif args.markdown:
        out.write(expectancy_strength_to_markdown(result))
    else:
