...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.write_report:
        report_path = write_owner_run_sanitized_telemetry_capture_attempt_report(
            result,
            args.report_path,
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        allow_abbrev=False,
        description=(
            "AIOS OANDA demo owner-run sanitized telemetry capture attempt V1."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Owner-only read-only OANDA demo telemetry capture. This never "
            "places, closes, or mutates orders."
        ),
    )
    return parser
