"""Run the local-only persistent profitability evidence adapter V1."""
...
def main(argv: list[str] | None = None, stdout: TextIO | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Persistent Profitability Evidence V1 sample.")
    ...
    sample = build_sample_persistent_profitability_summary()
    if args.blocked:
        sample["expectancy"] = -0.1
    if args.incomplete:
        sample.pop("closed_trade_count", None)
    result = evaluate_persistent_profitability_evidence(sample)
