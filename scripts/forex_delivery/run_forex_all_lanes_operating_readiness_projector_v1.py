"""CLI runner for the Forex all-lanes operating readiness projector."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Forex all-lanes operating readiness projector")
    ...
    parser.add_argument(
        "--report-path",
        default="Reports/forex_delivery/AIOS_FOREX_ALL_LANES_OPERATING_READINESS_PROJECTOR_V1_REPORT.md",
    )
...
def run_cli(argv: list[str] | None = None) -> str:
    ...
    result = projector_lib.project_all_lanes_operating_readiness(router)
    output = (
        json.dumps(projector_lib.projector_to_jsonable_dict(result), indent=2, sort_keys=True)
        if args.json
        else projector_lib.projector_to_markdown(result)
    )
    if args.write_report:
        ...
        report_path.write_text(projector_lib.projector_to_markdown(result), encoding="utf-8")
    return output
