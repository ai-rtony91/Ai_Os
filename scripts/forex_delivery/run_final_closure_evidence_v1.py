"""Run the local-only final closure evidence adapter V1."""
...
def main(argv: list[str] | None = None, stdout: TextIO | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Final Closure Evidence V1 sample.")
    ...
    sample = build_sample_final_closure_inputs()
    if args.blocked:
        sample["replay_evidence"]["status"] = "REPLAY_PROOF_BLOCKED"
        sample["replay_evidence"]["blockers"] = ["replay evidence is stale"]
    if args.incomplete:
        sample.pop("replay_evidence", None)
    result = evaluate_final_closure_evidence(sample)
