"""CLI runner for the Forex all-lanes owner boundary gate."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Forex all-lanes owner boundary gate")
    ...
    parser.add_argument(
        "--report-path",
        default="Reports/forex_delivery/AIOS_FOREX_ALL_LANES_OWNER_BOUNDARY_GATE_V1_REPORT.md",
    )
...
def run_cli(argv: list[str] | None = None) -> str:
    ...
    result = owner_gate_lib.evaluate_all_lanes_owner_boundary(projector)
    output = (
        json.dumps(owner_gate_lib.owner_boundary_to_jsonable_dict(result), indent=2, sort_keys=True)
        if args.json
        else owner_gate_lib.owner_boundary_to_markdown(result)
    )
    if args.write_report:
        ...
        report_path.write_text(owner_gate_lib.owner_boundary_to_markdown(result), encoding="utf-8")
    return output
