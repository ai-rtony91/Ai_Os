...
from automation.forex_engine.forex_vacation_mode_readiness_orchestrator_v1 import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preview build-only AIOS Forex Vacation Mode readiness."
    )
...
def main() -> int:
    args = parse_args()
    result = evaluate_forex_vacation_mode_readiness(_select_sample(args))
    if args.json:
