"""Runner for the AIOS owner micro-live exception approval grant packet."""
...
from automation.forex_engine.forex_owner_micro_live_exception_approval_grant_v1 import (  # noqa: E402
    CURRENT_STAGE,
...
STATE_PATH = Path(
    "Reports/forex_delivery/AIOS_FOREX_OWNER_MICRO_LIVE_EXCEPTION_APPROVAL_GRANT_V1_STATE.json",
)
REPORT_PATH = Path(
    "Reports/forex_delivery/AIOS_FOREX_OWNER_MICRO_LIVE_EXCEPTION_APPROVAL_GRANT_V1_REPORT.md",
)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate owner micro-live exception approval grant status.",
        allow_abbrev=False,
    )
    parser.add_argument(
        "--state-output",
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = run_forex_owner_micro_live_exception_approval_grant_v1(
...
def run_forex_owner_micro_live_exception_approval_grant_v1(
    *,
    ...
    result = evaluate_owner_micro_live_exception_approval_grant_v1(input_data)
    payload = {
        "packet_id": "PKT-FOREX-OWNER-MICRO-LIVE-EXCEPTION-APPROVAL-GRANT-V1",
        "module": "forex_owner_micro_live_exception_approval_grant_v1",
...
def _build_summary(result: OwnerMicroLiveExceptionApprovalGrantResult) -> dict[str, Any]:
    return {
        "default_mode": "dry_run",
        "current_stage": CURRENT_STAGE,
        "approval_status": result.owner_approval_status,
...
def _build_report_text(payload: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"- prior_approval_gate_landed: {str(result['prior_approval_gate_landed']).lower()}\n"
        ...
        f"- owner_confirms_future_runner_only: {str(result['owner_confirms_future_runner_only']).lower()}\n\n"
...
__all__ = [
    "run_forex_owner_micro_live_exception_approval_grant_v1",
    "parse_args",
]
