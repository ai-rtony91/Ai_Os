...
from automation.forex_engine.oanda_live_microtrade_profit_proof_candidate_review_v1 import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Review a selected profit proof packet preview as evidence only."
    )
...
def _select_sample(args: argparse.Namespace):
    if args.sample_loss:
        return build_sample_loss_review_input()
    if args.sample_breakeven:
        return build_sample_breakeven_review_input()
    if args.sample_missing:
        return build_sample_missing_owner_result_review_input()
    if args.sample_unsafe:
        return build_sample_unsafe_review_input()
    return build_sample_profit_review_input()


def main() -> int:
    args = parse_args()
    result = review_oanda_live_microtrade_profit_proof_candidate(_select_sample(args))
    if args.json:
