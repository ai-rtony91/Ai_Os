...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_sanitized_evidence_normalizer_acceptance(
        evidence,
        evidence_file_path=args.evidence_file,
    )
    report_path = None
    if args.write_report:
        report_path = write_sanitized_evidence_normalizer_acceptance_report(
            result,
            args.report_path,
    ...
    payload = {
        "script_status": result["acceptance_status"],
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        allow_abbrev=False,
        description=(
            "AIOS OANDA demo sanitized evidence normalizer acceptance run V1."
        ),
    )
...
def _load_evidence(evidence_file: str | None) -> dict[str, Any] | None:
