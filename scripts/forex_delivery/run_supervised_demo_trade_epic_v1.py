...
from automation.forex_engine.supervised_demo_trade_epic_v1 import (
    ...
    supervised_demo_epic_to_operator_text,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the local-only supervised demo trade epic sample.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    sample_input = (
        ...
        else build_sample_supervised_demo_ready_input()
    )
    result = run_supervised_demo_trade_epic(sample_input)
    if args.json:
        print(json.dumps(supervised_demo_epic_to_jsonable_dict(result), indent=2, sort_keys=True))
    elif args.markdown:
        print(supervised_demo_epic_to_markdown(result))
    else:
        print(supervised_demo_epic_to_operator_text(result))
    return 0
