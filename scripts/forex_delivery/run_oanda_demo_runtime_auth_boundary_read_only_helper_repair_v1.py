...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = evaluate_runtime_auth_boundary_read_only_helper_repair(evidence)
    report_path = None
    if args.write_report:
        report_path = write_runtime_auth_boundary_read_only_helper_repair_report(
            result,
            args.report_path,
    ...
    payload = {
        "script_status": result["runtime_auth_boundary_status"],
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        allow_abbrev=False,
        description=(
            "AIOS OANDA demo runtime auth boundary read-only helper repair V1."
        ),
    )
