...
from automation.forex_engine.supervised_demo_trade_review_bundle_v1 import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the supervised demo trade review bundle V1 sample.")
    ...
    bundle_input = (
        ...
        else build_sample_supervised_demo_trade_review_bundle_ready_input()
    )
    result = build_supervised_demo_trade_review_bundle(bundle_input)

    if args.json:
        print(json.dumps(supervised_demo_trade_review_bundle_to_jsonable_dict(result), indent=2, sort_keys=True))
        return 0
    if args.markdown:
        print(supervised_demo_trade_review_bundle_to_markdown(result))
        return 0
    print(supervised_demo_trade_review_bundle_to_operator_text(result))
    return 0
