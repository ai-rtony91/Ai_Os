"""CLI runner for local-only AIOS Forex Demo Review Engine V1."""
...
from automation.forex_engine.demo_review_engine_v1 import (  # noqa: E402
    ...
    build_sample_mixed_demo_review_input,
...
def main(argv: list[str] | None = None, stdout: TextIO | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run local-only AIOS Forex Demo Review Engine V1."
    )
    ...
    promotion = (
        ...
        else build_sample_mixed_demo_review_input()
    )
    result = evaluate_demo_review(promotion)
    out = stdout if stdout is not None else sys.stdout
    if args.json:
        json.dump(result_to_jsonable_dict(result), out, indent=2, sort_keys=True)
        out.write("\n")
    elif args.markdown:
        out.write(demo_review_to_markdown(result))
    else:
