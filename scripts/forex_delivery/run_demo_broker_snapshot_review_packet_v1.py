...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the local demo broker snapshot review packet sample.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    sample_input = (
        ...
        else build_sample_review_packet_input()
    )
    result = build_demo_broker_snapshot_review_packet(sample_input)
    if args.json:
        print(json.dumps(demo_broker_snapshot_review_packet_to_jsonable_dict(result), indent=2, sort_keys=True))
    elif args.markdown:
        print(demo_broker_snapshot_review_packet_to_markdown(result))
    else:
        print(demo_broker_snapshot_review_packet_to_operator_text(result))
    return 0
