...
from automation.forex_engine.oanda_live_microtrade_result_to_next_proof_router_v1 import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preview the next proof lane for one captured owner-run result."
    )
...
def _select_sample(args: argparse.Namespace):
    if args.sample_loss:
        return build_sample_loss_router_input()
    if args.sample_breakeven:
        return build_sample_breakeven_router_input()
    if args.sample_missing:
        return build_sample_missing_owner_result_router_input()
    if args.sample_unsafe:
        return build_sample_unsafe_router_input()
    return build_sample_profit_router_input()


def main() -> int:
    args = parse_args()
    result = route_oanda_live_microtrade_result_to_next_proof(_select_sample(args))
    if args.json:
