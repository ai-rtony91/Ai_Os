...
from automation.forex_engine.oanda_demo_connection_probe import (
    PROBE_MODE as OANDA_DEMO_PROBE_MODE,
    PROBE_SCOPE as OANDA_DEMO_PROBE_SCOPE,
    evaluate_oanda_demo_connection_probe,
)
...
DEMO_CONNECTION_PREFLIGHT_REQUIRED_FIELDS = (
    "broker_family_label",
    "demo_practice_mode_confirmed",
    "runtime_auth_reference_label",
    "external_connector_readiness_flag",
    "protected_action_approval_status",
    "network_approval_status",
    "endpoint_class",
    "account_identifier_status",
...
DEMO_CONNECTION_CREDENTIAL_MATERIAL_STATUSES = (
    "ABSENT",
)
...
DEMO_CONNECTION_PROTECTED_ACTION_GATE_REQUIRED_FIELDS = (
    "request_draft_status",
    "broker_family_label",
    "demo_practice_mode_confirmed",
    "runtime_auth_reference_label",
    "external_connector_readiness_flag",
    "protected_action_review",
    "human_owner_review",
    "approval_mutation",
    "network_execution",
    "credential_material_status",
...
DEMO_CONNECTION_PROTECTED_ACTION_GATE_REJECTED = "REJECTED"
DEMO_CONNECTION_PROTECTED_ACTION_GATE_INCOMPLETE = "INCOMPLETE"
DEMO_CONNECTION_PROTECTED_ACTION_GATE_REVIEW_READY = "REVIEW_READY"

DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_REQUIRED_FIELDS = (
    "protected_action_gate_status",
    "request_draft_status",
    "broker_family_label",
    "demo_practice_mode_confirmed",
    "runtime_auth_reference_label",
...
DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_REJECTED = "REJECTED"
DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_INCOMPLETE = "INCOMPLETE"
DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_READY = "DRAFT_READY_FOR_HUMAN_REVIEW"

DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_REQUIRED_FIELDS = (
    "execution_packet_draft_status",
    "protected_action_gate_status",
    "request_draft_status",
    "broker_family_label",
    "demo_practice_mode_confirmed",
...
DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_REJECTED = "REJECTED"
DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_INCOMPLETE = "INCOMPLETE"
DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_READY = "APPROVAL_REVIEW_READY"

DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_REQUIRED_FIELDS = (
...
DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_REJECTED = "REJECTED"
DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_INCOMPLETE = "INCOMPLETE"
DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_READY = (
    "APPROVAL_RECORD_DRAFT_READY"
)

DEMO_CONNECTION_EXECUTION_COMMAND_PLACEHOLDERS = (
    "NO_COMMAND_DRAFT_ONLY",
    "NO_CONNECTOR_COMMAND_DRAFT_ONLY",
    "SEPARATE_PACKET_REQUIRED",
    "SEPARATE_PROTECTED_ACTION_APPROVAL_REQUIRED",
)

REAL_COMMAND_MARKERS = (
    "python ",
    "python.exe",
    "powershell",
    "pwsh",
...
DEMO_CONNECTION_APPROVAL_RECORD_ID_PLACEHOLDERS = (
    "PENDING_SEPARATE_APPROVAL_RECORD_ID",
    "NO_APPROVAL_RECORD_ID_DRAFT_ONLY",
    "SEPARATE_HUMAN_OWNER_APPROVAL_RECORD_REQUIRED",
)

DEMO_CONNECTION_APPROVAL_TIMESTAMP_PLACEHOLDERS = (
    "PENDING_SEPARATE_APPROVAL_TIMESTAMP",
    "NO_APPROVAL_TIMESTAMP_DRAFT_ONLY",
    "SEPARATE_HUMAN_OWNER_APPROVAL_TIMESTAMP_REQUIRED",
)
...
def build_demo_connection_proof_preflight_dry_run(
    preflight_fields: dict[str, Any],
) -> dict[str, Any]:
    """Validate a future demo connection proof preflight without executing it."""

    fields = dict(preflight_fields or {})
    forbidden = _find_forbidden_fields(fields)
    missing = [
        field_name
        for field_name in DEMO_CONNECTION_PREFLIGHT_REQUIRED_FIELDS
        if not _field_present(fields, field_name)
    ]
    blocker_reasons = [f"missing_required_field:{field}" for field in missing]
    blocker_reasons.extend([f"forbidden_field:{field}" for field in forbidden])
    ...
    if missing:
        failed_gates.append("required_demo_connection_preflight_fields_present")
    else:
        passed_gates.append("required_demo_connection_preflight_fields_present")

    if forbidden:
        failed_gates.append("no_forbidden_demo_connection_preflight_fields")
    else:
        passed_gates.append("no_forbidden_demo_connection_preflight_fields")
    ...
    if fields.get("demo_practice_mode_confirmed") is True:
        ...
        blocker_reasons.append("demo_practice_mode_confirmation_required")
    ...
    if fields.get("protected_action_approval_status") is False:
        passed_gates.append("protected_action_approval_status")
    else:
        failed_gates.append("protected_action_approval_status")
        blocker_reasons.append(
            "protected_action_approval_status_must_remain_false_for_dry_run"
        )
    ...
    if credential_material_status in DEMO_CONNECTION_CREDENTIAL_MATERIAL_STATUSES:
        ...
        blocker_reasons.append("credential_material_status_must_be_absent")
    ...
    if fields.get("one_shot_stop_requirement") is True:
        ...
        blocker_reasons.append("one_shot_stop_requirement_must_be_confirmed")
    ...
    if retry_count == 0:
        ...
        blocker_reasons.append("retry_count_must_be_zero")
    ...
    runtime_readiness = build_demo_runtime_readiness_dry_run(
        {
            "broker_family_label": fields.get("broker_family_label"),
            "practice_demo_mode_confirmed": fields.get("demo_practice_mode_confirmed"),
            ...
            "no_order_route_approval": fields.get("order_route_approval") is False,
            "no_live_endpoint": "LIVE" not in endpoint_class,
            "no_credential_value": fields.get("credential_material_status") == "ABSENT",
            "no_account_id_value": account_identifier_status
            in DEMO_RUNTIME_ACCOUNT_IDENTIFIER_STATUSES,
            ...
            "human_owner_approval_future_proof": fields.get(
                "human_owner_approval_future_proof"
            ),
        }
    )

    if runtime_readiness.get("demo_runtime_ready") is True:
        passed_gates.append("demo_runtime_readiness")
    else:
        failed_gates.append("demo_runtime_readiness")
        blocker_reasons.append("demo_runtime_readiness_required")
        blocker_reasons.extend(
            f"demo_runtime_blocker:{blocker}"
            for blocker in list(runtime_readiness.get("blocker_reasons") or [])
        )

    endpoint_classification = _demo_runtime_endpoint_classification(endpoint_class)
    connection_probe_preview = evaluate_oanda_demo_connection_probe(
        {
            "broker_id": "OANDA",
            "account_mode": "PRACTICE_DEMO",
            "environment": endpoint_classification,
            "endpoint_classification": endpoint_classification,
            "probe_scope": OANDA_DEMO_PROBE_SCOPE,
            "probe_mode": OANDA_DEMO_PROBE_MODE,
            "demo_probe_approval_flag": fields.get("protected_action_approval_status")
            is True,
            "network_broker_call_gate_approved": fields.get("network_approval_status")
            is True,
            "runtime_auth_reference_present": _field_present(
                fields, "runtime_auth_reference_label"
            ),
            "runtime_auth_reference_format": OANDA_RUNTIME_AUTH_REFERENCE_FORMAT,
            "auth_material_location": OANDA_RUNTIME_AUTH_MATERIAL_LOCATION,
            "runtime_auth_boundary_confirmed": True,
            "repo_storage_confirmed_absent": True,
            "account_identifier_present": False,
            "one_shot_only": fields.get("one_shot_stop_requirement") is True,
            "timeout_seconds": timeout_seconds,
            "stop_on_success_or_failure": fields.get("one_shot_stop_requirement")
            is True,
            "no_order_route_confirmed": fields.get("order_route_approval") is False,
            "no_account_id_logging_confirmed": account_identifier_status
            in DEMO_RUNTIME_ACCOUNT_IDENTIFIER_STATUSES,
            "audit_logging_acknowledged": True,
        }
    )
    ...
    failed_gates = sorted(set(failed_gates))
    blocker_reasons = sorted(set(blocker_reasons))
    preflight_ready = not blocker_reasons

    return {
        "schema": "AIOS_FOREX_DELIVERY_DEMO_CONNECTION_PROOF_PREFLIGHT_DRY_RUN.v1",
        "demo_connection_preflight_ready": preflight_ready,
        "future_demo_connection_proof_review_ready": preflight_ready,
        "dry_run_only": True,
        ...
        "live_endpoint_allowed": False,
        "live_execution_allowed": False,
        "required_fields": list(DEMO_CONNECTION_PREFLIGHT_REQUIRED_FIELDS),
        "missing_fields": missing,
        "passed_gates": passed_gates,
        "failed_gates": failed_gates,
        "blocker_reasons": blocker_reasons,
        "demo_runtime_readiness": runtime_readiness,
        "connection_probe_non_executable_preview": connection_probe_preview,
        "next_required_action": (
            "future_human_owner_demo_connection_proof_approval_review"
            if preflight_ready
            else "complete_sanitized_demo_connection_proof_preflight_fields"
        ),
        "stop_point": (
            "Preflight dry-run validation only; no protected action, broker connection, "
            "credential access, network call, market-data fetch, paper order, or live "
            "order is authorized."
        ),
    }
...
def build_demo_connection_proof_protected_action_gate_dry_run(
    gate_fields: dict[str, Any],
) -> dict[str, Any]:
    """Classify protected-action gate readiness without granting approval."""

    fields = dict(gate_fields or {})
    forbidden = _find_forbidden_fields(fields)
    missing = [
        field_name
        for field_name in DEMO_CONNECTION_PROTECTED_ACTION_GATE_REQUIRED_FIELDS
        if not _field_present(fields, field_name)
    ]
    incomplete_reasons = [f"missing_required_field:{field}" for field in missing]
    rejected_reasons = [f"forbidden_field:{field}" for field in forbidden]
    ...
    if missing:
        failed_gates.append("required_demo_connection_protected_action_gate_fields_present")
    else:
        passed_gates.append("required_demo_connection_protected_action_gate_fields_present")

    if forbidden:
        failed_gates.append("no_forbidden_demo_connection_protected_action_gate_fields")
    else:
        passed_gates.append("no_forbidden_demo_connection_protected_action_gate_fields")

    request_draft_status = str(fields.get("request_draft_status") or "")
    broker_family = _normalize_demo_runtime_label(fields.get("broker_family_label"))
    endpoint_class = _normalize_demo_runtime_label(fields.get("endpoint_class"))
    account_identifier_status = _normalize_demo_runtime_label(
        fields.get("account_identifier_status")
    ...
    if request_draft_status == DEMO_CONNECTION_REQUEST_DRAFT_READY:
        passed_gates.append("request_draft_status")
    elif _field_present(fields, "request_draft_status"):
        failed_gates.append("request_draft_status")
        rejected_reasons.append("request_draft_status_must_be_draft_ready")
    else:
        failed_gates.append("request_draft_status")
        incomplete_reasons.append("request_draft_status_required")
    ...
    protected_action_review_unsafe = _unsafe_text_value_reasons(
        fields.get("protected_action_review")
    )
    if _field_present(fields, "protected_action_review") and not protected_action_review_unsafe:
        passed_gates.append("protected_action_review")
    else:
        failed_gates.append("protected_action_review")
        if _field_present(fields, "protected_action_review"):
            rejected_reasons.extend(
                f"unsafe_protected_action_review:{reason}"
                for reason in protected_action_review_unsafe
            )
        else:
            incomplete_reasons.append("protected_action_review_required")
    ...
    if fields.get("approval_mutation") is False:
        passed_gates.append("approval_mutation")
    elif _field_present(fields, "approval_mutation"):
        failed_gates.append("approval_mutation")
        rejected_reasons.append("approval_mutation_must_be_false")
    else:
        failed_gates.append("approval_mutation")

    if fields.get("network_execution") is False:
        passed_gates.append("network_execution")
    elif _field_present(fields, "network_execution"):
        failed_gates.append("network_execution")
        rejected_reasons.append("network_execution_must_be_false")
    else:
        failed_gates.append("network_execution")
    ...
    request_draft_preview = build_demo_connection_proof_request_draft_dry_run(
        {
            "broker_family_label": fields.get("broker_family_label"),
            "demo_practice_mode_confirmed": fields.get("demo_practice_mode_confirmed"),
            "runtime_auth_reference_label": fields.get("runtime_auth_reference_label"),
    ...
    if request_draft_preview.get("request_draft_classification") == DEMO_CONNECTION_REQUEST_DRAFT_READY:
        passed_gates.append("request_draft_preview")
    else:
        failed_gates.append("request_draft_preview")
        if rejected_reasons:
            rejected_reasons.append("request_draft_preview_rejected")
        else:
            incomplete_reasons.append("request_draft_preview_required")
    ...
    if rejected_reasons:
        classification = DEMO_CONNECTION_PROTECTED_ACTION_GATE_REJECTED
    elif incomplete_reasons:
        classification = DEMO_CONNECTION_PROTECTED_ACTION_GATE_INCOMPLETE
    else:
        classification = DEMO_CONNECTION_PROTECTED_ACTION_GATE_REVIEW_READY

    return {
        "schema": "AIOS_FOREX_DELIVERY_DEMO_CONNECTION_PROOF_PROTECTED_ACTION_GATE_DRY_RUN.v1",
        "protected_action_gate_classification": classification,
        "review_ready": classification
        == DEMO_CONNECTION_PROTECTED_ACTION_GATE_REVIEW_READY,
        "dry_run_only": True,
        "proof_executable_now": False,
        "approval_state_mutated": False,
        "approval_state_changed": False,
        ...
        "live_endpoint_allowed": False,
        "live_execution_allowed": False,
        "required_fields": list(DEMO_CONNECTION_PROTECTED_ACTION_GATE_REQUIRED_FIELDS),
        "missing_fields": missing,
        "passed_gates": passed_gates,
        "failed_gates": failed_gates,
        "incomplete_reasons": incomplete_reasons,
        "rejected_reasons": rejected_reasons,
        "request_draft_preview": request_draft_preview,
        "next_required_action": (
            "human_owner_protected_action_gate_review_only"
            if classification == DEMO_CONNECTION_PROTECTED_ACTION_GATE_REVIEW_READY
            else "repair_sanitized_protected_action_gate_request"
        ),
        "stop_point": (
            "Protected-action gate dry-run only; no approval is granted or mutated, "
            "and no broker connection, credential access, network call, market-data "
            "fetch, paper order, or live order is authorized."
        ),
    }


def build_demo_connection_proof_execution_packet_draft_dry_run(
    execution_fields: dict[str, Any],
) -> dict[str, Any]:
    """Draft a future proof execution packet without making it executable."""

    fields = dict(execution_fields or {})
    forbidden = _find_forbidden_fields(fields)
    missing = [
        field_name
        for field_name in DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_REQUIRED_FIELDS
        if not _field_present(fields, field_name)
    ]
    incomplete_reasons = [f"missing_required_field:{field}" for field in missing]
    rejected_reasons = [f"forbidden_field:{field}" for field in forbidden]
    ...
    if missing:
        failed_gates.append("required_demo_connection_execution_packet_draft_fields_present")
    else:
        passed_gates.append(
            "required_demo_connection_execution_packet_draft_fields_present"
        )

    if forbidden:
        failed_gates.append("no_forbidden_demo_connection_execution_packet_draft_fields")
    else:
        passed_gates.append("no_forbidden_demo_connection_execution_packet_draft_fields")

    protected_gate_status = str(fields.get("protected_action_gate_status") or "")
    request_draft_status = str(fields.get("request_draft_status") or "")
    broker_family = _normalize_demo_runtime_label(fields.get("broker_family_label"))
    endpoint_class = _normalize_demo_runtime_label(fields.get("endpoint_class"))
    account_identifier_status = _normalize_demo_runtime_label(
    ...
    if protected_gate_status == DEMO_CONNECTION_PROTECTED_ACTION_GATE_REVIEW_READY:
        passed_gates.append("protected_action_gate_status")
    elif _field_present(fields, "protected_action_gate_status"):
        failed_gates.append("protected_action_gate_status")
        rejected_reasons.append("protected_action_gate_status_must_be_review_ready")
    else:
        failed_gates.append("protected_action_gate_status")
        incomplete_reasons.append("protected_action_gate_status_required")
    ...
    approval_value = str(fields.get("human_owner_approval") or "").strip()
    if approval_value in (
        "PENDING_SEPARATE_HUMAN_OWNER_APPROVAL",
        "SEPARATE_HUMAN_OWNER_APPROVAL_REQUIRED",
        "NOT_GRANTED_DRAFT_ONLY",
    ):
        passed_gates.append("human_owner_approval")
    elif _field_present(fields, "human_owner_approval"):
        failed_gates.append("human_owner_approval")
        rejected_reasons.append("human_owner_approval_must_remain_placeholder_only")
    else:
        failed_gates.append("human_owner_approval")
        incomplete_reasons.append("human_owner_approval_placeholder_required")
    ...
    for field_name in ("execution_command", "connector_command"):
        if _command_placeholder_only(fields.get(field_name)):
            passed_gates.append(field_name)
        elif _field_present(fields, field_name):
            failed_gates.append(field_name)
            rejected_reasons.append(f"{field_name}_must_remain_placeholder_only")
        else:
            failed_gates.append(field_name)
            incomplete_reasons.append(f"{field_name}_placeholder_required")
    ...
    for field_name, reason in (
        ...
        ("broker_connection_allowed", "broker_connection_permission_blocked"),
        ("connection_attempt_requested", "connection_attempt_request_blocked"),
        ("command_execution_requested", "command_execution_request_blocked"),
        ("connector_command_requested", "connector_command_request_blocked"),
        ("retry_loop_requested", "retry_loop_request_blocked"),
        ("autonomous_reentry", "autonomous_reentry_blocked"),
    ):
        if fields.get(field_name) is True:
    ...
    protected_gate_preview = build_demo_connection_proof_protected_action_gate_dry_run(
        {
            "request_draft_status": fields.get("request_draft_status"),
            "broker_family_label": fields.get("broker_family_label"),
            "demo_practice_mode_confirmed": fields.get("demo_practice_mode_confirmed"),
            "runtime_auth_reference_label": fields.get("runtime_auth_reference_label"),
            "external_connector_readiness_flag": fields.get(
            ...
            "protected_action_review": fields.get("protected_action_review"),
            "human_owner_review": fields.get("human_owner_review"),
            "approval_mutation": fields.get("approval_mutation"),
            "network_execution": fields.get("network_execution"),
            "credential_material_status": fields.get("credential_material_status"),
            "account_identifier_status": fields.get("account_identifier_status"),
            "endpoint_class": fields.get("endpoint_class"),
            "order_route_approval": fields.get("order_route_approval"),
    ...
    if (
        protected_gate_preview.get("protected_action_gate_classification")
        == DEMO_CONNECTION_PROTECTED_ACTION_GATE_REVIEW_READY
    ):
        passed_gates.append("protected_action_gate_preview")
    else:
        failed_gates.append("protected_action_gate_preview")
        if rejected_reasons:
            rejected_reasons.append("protected_action_gate_preview_rejected")
        else:
            incomplete_reasons.append("protected_action_gate_preview_required")
    ...
    if rejected_reasons:
        classification = DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_REJECTED
    elif incomplete_reasons:
        classification = DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_INCOMPLETE
    else:
        classification = DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_READY

    return {
        "schema": (
            "AIOS_FOREX_DELIVERY_DEMO_CONNECTION_PROOF_EXECUTION_PACKET_DRAFT_DRY_RUN.v1"
        ),
        "execution_packet_draft_classification": classification,
        "draft_ready_for_human_review": (
            classification == DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_READY
        ),
        "dry_run_only": True,
        "execution_packet_executable_now": False,
        "proof_executable_now": False,
        "approval_state_mutated": False,
        "approval_state_changed": False,
        "protected_action_approval_granted": False,
        ...
        "connector_command_allowed": False,
        "connector_command_executed": False,
        "shell_used": False,
        "broker_connection_allowed": False,
        "connection_attempt_allowed": False,
        "connection_attempt_performed": False,
        ...
        "live_endpoint_allowed": False,
        "live_execution_allowed": False,
        "required_fields": list(DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_REQUIRED_FIELDS),
        "missing_fields": missing,
        "passed_gates": passed_gates,
        "failed_gates": failed_gates,
        "incomplete_reasons": incomplete_reasons,
        "rejected_reasons": rejected_reasons,
        "protected_action_gate_preview": protected_gate_preview,
        "next_required_action": (
            "human_owner_review_execution_packet_draft_only"
            if classification == DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_READY
            else "repair_sanitized_execution_packet_draft"
        ),
        "stop_point": (
            "Execution-packet draft dry-run only; no command is executable, no "
            "approval is granted or mutated, and no broker connection, credential "
            "access, network call, market-data fetch, paper order, or live order is "
            "authorized."
        ),
    }


def build_demo_connection_proof_protected_action_approval_review_dry_run(
    approval_fields: dict[str, Any],
) -> dict[str, Any]:
    """Review protected-action approval readiness without granting approval."""

    fields = dict(approval_fields or {})
    forbidden = _find_forbidden_fields(fields)
    missing = [
        field_name
        for field_name in DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_REQUIRED_FIELDS
        if not _field_present(fields, field_name)
    ]
    incomplete_reasons = [f"missing_required_field:{field}" for field in missing]
    rejected_reasons = [f"forbidden_field:{field}" for field in forbidden]
    ...
    if missing:
        failed_gates.append("required_demo_connection_protected_action_approval_fields_present")
    else:
        passed_gates.append("required_demo_connection_protected_action_approval_fields_present")

    if forbidden:
        failed_gates.append("no_forbidden_demo_connection_protected_action_approval_fields")
    else:
        passed_gates.append("no_forbidden_demo_connection_protected_action_approval_fields")

    execution_draft_status = str(fields.get("execution_packet_draft_status") or "")
    protected_gate_status = str(fields.get("protected_action_gate_status") or "")
    request_draft_status = str(fields.get("request_draft_status") or "")
    broker_family = _normalize_demo_runtime_label(fields.get("broker_family_label"))
    endpoint_class = _normalize_demo_runtime_label(fields.get("endpoint_class"))
    ...
    if execution_draft_status == DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_READY:
        passed_gates.append("execution_packet_draft_status")
    elif _field_present(fields, "execution_packet_draft_status"):
        failed_gates.append("execution_packet_draft_status")
        rejected_reasons.append(
            "execution_packet_draft_status_must_be_draft_ready_for_human_review"
        )
    else:
        failed_gates.append("execution_packet_draft_status")
        incomplete_reasons.append("execution_packet_draft_status_required")
    ...
    for field_name, reason in (
        (
            "protected_action_approval_requested",
            "protected_action_approval_request_must_be_true",
        ),
        ("network_approval_requested", "network_approval_request_must_be_true"),
    ):
        if fields.get(field_name) is True:
            passed_gates.append(field_name)
        else:
    ...
    execution_packet_draft_preview = (
        build_demo_connection_proof_execution_packet_draft_dry_run(
            {
                "protected_action_gate_status": fields.get("protected_action_gate_status"),
                "request_draft_status": fields.get("request_draft_status"),
                "broker_family_label": fields.get("broker_family_label"),
                "demo_practice_mode_confirmed": fields.get(
                    "demo_practice_mode_confirmed"
                ),
                "runtime_auth_reference_label": fields.get(
                    "runtime_auth_reference_label"
                ),
                "external_connector_readiness_flag": fields.get(
                    "external_connector_readiness_flag"
                ...
                "protected_action_review": fields.get("protected_action_review"),
                "human_owner_review": fields.get("human_owner_review"),
                "human_owner_approval": fields.get("human_owner_approval"),
                "approval_mutation": fields.get("approval_mutation"),
                "network_execution": fields.get("network_execution"),
                "execution_command": fields.get("execution_command"),
                "connector_command": fields.get("connector_command"),
                "credential_material_status": fields.get("credential_material_status"),
                "account_identifier_status": fields.get("account_identifier_status"),
                "endpoint_class": fields.get("endpoint_class"),
                "order_route_approval": fields.get("order_route_approval"),
    ...
    if (
        execution_packet_draft_preview.get("execution_packet_draft_classification")
        == DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_READY
    ):
        passed_gates.append("execution_packet_draft_preview")
    else:
        failed_gates.append("execution_packet_draft_preview")
        if rejected_reasons:
            rejected_reasons.append("execution_packet_draft_preview_rejected")
        else:
            incomplete_reasons.append("execution_packet_draft_preview_required")
    ...
    if rejected_reasons:
        classification = DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_REJECTED
    elif incomplete_reasons:
        classification = DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_INCOMPLETE
    else:
        classification = DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_READY

    return {
        "schema": (
            "AIOS_FOREX_DELIVERY_DEMO_CONNECTION_PROOF_PROTECTED_ACTION_APPROVAL_REVIEW_DRY_RUN.v1"
        ),
        "protected_action_approval_review_classification": classification,
        ...
        "incomplete_reasons": incomplete_reasons,
        "rejected_reasons": rejected_reasons,
        "execution_packet_draft_preview": execution_packet_draft_preview,
        "next_required_action": (
            "human_owner_protected_action_approval_review_only"
            if classification
            == DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_READY
            else "repair_sanitized_protected_action_approval_review"
        ),
        "stop_point": (
            "Protected-action approval-review dry-run only; no approval is granted "
            "or mutated, no command is executable, and no broker connection, "
            "credential access, network call, market-data fetch, paper order, or "
            "live order is authorized."
        ),
    }


def build_demo_connection_proof_protected_action_approval_record_draft_dry_run(
    record_fields: dict[str, Any],
) -> dict[str, Any]:
    """Draft a protected-action approval record without authorizing it."""

    fields = dict(record_fields or {})
    forbidden = _find_forbidden_fields(fields)
    missing = [
        field_name
        for field_name in DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_REQUIRED_FIELDS
        if not _field_present(fields, field_name)
    ]
    incomplete_reasons = [f"missing_required_field:{field}" for field in missing]
    rejected_reasons = [f"forbidden_field:{field}" for field in forbidden]
    ...
    if missing:
        failed_gates.append("required_demo_connection_approval_record_draft_fields_present")
    else:
        passed_gates.append("required_demo_connection_approval_record_draft_fields_present")

    if forbidden:
        failed_gates.append("no_forbidden_demo_connection_approval_record_draft_fields")
    else:
        passed_gates.append("no_forbidden_demo_connection_approval_record_draft_fields")

    approval_review_status = str(fields.get("approval_review_status") or "")
    execution_draft_status = str(fields.get("execution_packet_draft_status") or "")
    protected_gate_status = str(fields.get("protected_action_gate_status") or "")
    request_draft_status = str(fields.get("request_draft_status") or "")
    broker_family = _normalize_demo_runtime_label(fields.get("broker_family_label"))
    ...
    if approval_review_status == DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_READY:
        passed_gates.append("approval_review_status")
    elif _field_present(fields, "approval_review_status"):
        failed_gates.append("approval_review_status")
        rejected_reasons.append("approval_review_status_must_be_approval_review_ready")
    else:
        failed_gates.append("approval_review_status")
        incomplete_reasons.append("approval_review_status_required")
    ...
    if _placeholder_only(
        fields.get("approval_record_id"),
        DEMO_CONNECTION_APPROVAL_RECORD_ID_PLACEHOLDERS,
    ):
        passed_gates.append("approval_record_id")
    elif _field_present(fields, "approval_record_id"):
        failed_gates.append("approval_record_id")
        rejected_reasons.append("approval_record_id_must_remain_placeholder_only")
    else:
        failed_gates.append("approval_record_id")
        incomplete_reasons.append("approval_record_id_placeholder_required")

    if _placeholder_only(
        fields.get("approval_timestamp"),
        DEMO_CONNECTION_APPROVAL_TIMESTAMP_PLACEHOLDERS,
    ):
        passed_gates.append("approval_timestamp")
    elif _field_present(fields, "approval_timestamp"):
        failed_gates.append("approval_timestamp")
        rejected_reasons.append("approval_timestamp_must_remain_placeholder_only")
    else:
        failed_gates.append("approval_timestamp")
        incomplete_reasons.append("approval_timestamp_placeholder_required")
    ...
    if (
        approval_review_preview.get("protected_action_approval_review_classification")
        == DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_READY
    ):
        passed_gates.append("approval_review_preview")
    else:
        failed_gates.append("approval_review_preview")
        if rejected_reasons:
            rejected_reasons.append("approval_review_preview_rejected")
        else:
            incomplete_reasons.append("approval_review_preview_required")
    ...
    if rejected_reasons:
        classification = DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_REJECTED
    elif incomplete_reasons:
        classification = DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_INCOMPLETE
    else:
        classification = DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_READY

    return {
        "schema": (
            "AIOS_FOREX_DELIVERY_DEMO_CONNECTION_PROOF_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_DRY_RUN.v1"
        ),
        "approval_record_draft_classification": classification,
        "approval_record_draft_ready": (
            classification
            == DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_READY
        ),
        "dry_run_only": True,
        "approval_record_authorizing": False,
        ...
        "incomplete_reasons": incomplete_reasons,
        "rejected_reasons": rejected_reasons,
        "approval_review_preview": approval_review_preview,
        "next_required_action": (
            "human_owner_review_approval_record_draft_only"
            if classification
            == DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_READY
            else "repair_sanitized_approval_record_draft"
        ),
        "stop_point": (
            "Approval-record draft dry-run only; no approval record is created, "
            "persisted, authorizing, granted, or mutated, no command is executable, "
            "and no broker connection, credential access, network call, market-data "
            "fetch, paper order, or live order is authorized."
        ),
    }
...
def _forbidden_key_name(key_text: str) -> bool:
    ...
    if key_text in DEMO_CONNECTION_REQUEST_DRAFT_REQUIRED_FIELDS:
        return False
    if key_text in DEMO_CONNECTION_PROTECTED_ACTION_GATE_REQUIRED_FIELDS:
        return False
    if key_text in DEMO_CONNECTION_EXECUTION_PACKET_DRAFT_REQUIRED_FIELDS:
        return False
    if key_text in DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_REVIEW_REQUIRED_FIELDS:
        return False
    if key_text in DEMO_CONNECTION_PROTECTED_ACTION_APPROVAL_RECORD_DRAFT_REQUIRED_FIELDS:
        return False
...
def _placeholder_only(value: Any, allowed_placeholders: tuple[str, ...]) -> bool:
    return isinstance(value, str) and value.strip() in allowed_placeholders


def _command_placeholder_only(value: Any) -> bool:
    if not isinstance(value, str):
        return False
    normalized = value.strip()
    lowered = normalized.lower()
    if normalized in DEMO_CONNECTION_EXECUTION_COMMAND_PLACEHOLDERS:
        return True
    if any(marker in lowered for marker in REAL_COMMAND_MARKERS):
        return False
    return False
