from __future__ import annotations

import argparse
import json
from pathlib import PurePosixPath
from typing import Any


SCHEMA = "AIOS_SELF_BUILD_APPLY_APPROVAL_GATE.v1"
AUTHORIZED_APPROVER = "Anthony Meza"
SANDBOX_1312_MARKER = "createprocessasuserw failed: 1312"

PROTECTED_ACTIONS_BLOCKED = {
    "git_add": True,
    "git_commit": True,
    "git_push": True,
    "merge": True,
    "scheduler_activation": True,
    "daemon_activation": True,
    "worker_dispatch": True,
    "queue_mutation": True,
    "approval_mutation": True,
    "broker_live_trading": True,
    "credentials": True,
    "real_orders": True,
    "real_webhooks": True,
    "destructive_cleanup": True,
}

PROTECTED_REQUEST_KEYS = {
    "git_add",
    "git_commit",
    "git_push",
    "merge",
    "scheduler_activation",
    "daemon_activation",
    "worker_dispatch",
    "queue_mutation",
    "approval_mutation",
    "broker_live_trading",
    "broker",
    "broker_order",
    "live_trading",
    "live_execution",
    "credentials",
    "credential_use",
    "real_orders",
    "real_order",
    "real_webhooks",
    "webhook_url",
    "destructive_cleanup",
    "destructive_action",
    "background_runtime",
    "network",
    "network_access",
    "codex_launch",
    "chatgpt_api_call",
    "command_execution",
    "commands_executed",
}

REQUESTED_PROTECTED_KEYS = {
    "git_add_requested",
    "git_commit_requested",
    "git_push_requested",
    "merge_requested",
    "scheduler_activation_requested",
    "daemon_activation_requested",
    "worker_dispatch_requested",
    "queue_mutation_requested",
    "approval_mutation_requested",
    "broker_live_trading_requested",
    "credentials_requested",
    "real_orders_requested",
    "real_webhooks_requested",
    "destructive_cleanup_requested",
    "background_runtime_requested",
    "network_access_requested",
    "codex_launch_requested",
    "chatgpt_api_call_requested",
}


def _base_safety() -> dict[str, bool]:
    return {
        "gate_only": True,
        "files_written": False,
        "commands_executed": False,
        "network_access": False,
        "codex_launch": False,
        "chatgpt_api_call": False,
        "broker": False,
        "live_trading": False,
        "credentials": False,
        "real_orders": False,
        "real_webhooks": False,
        "scheduler": False,
        "daemon": False,
        "worker_dispatch": False,
        "queue_mutation": False,
        "approval_mutation": False,
        "git_add": False,
        "git_commit": False,
        "git_push": False,
        "merge": False,
        "destructive_action": False,
        "background_runtime": False,
        "protected_action_requested": False,
        "sandbox_1312_blocker": False,
    }


def _as_dict(value: Any) -> dict[str, Any]:
    return value if isinstance(value, dict) else {}


def _as_list(value: Any) -> list[Any]:
    return value if isinstance(value, list) else []


def _normalize_paths(paths: Any) -> list[str]:
    output: list[str] = []
    for path in _as_list(paths):
        path_text = str(path)
        parsed = PurePosixPath(path_text)
        if path_text and "\\" not in path_text and not parsed.is_absolute() and ".." not in parsed.parts:
            output.append(path_text)
    return output


def _first_present(*values: Any, default: Any = None) -> Any:
    for value in values:
        if value not in (None, ""):
            return value
    return default


def _iter_items(value: Any, path: tuple[str, ...] = ()) -> list[tuple[tuple[str, ...], str, Any]]:
    output: list[tuple[tuple[str, ...], str, Any]] = []
    if isinstance(value, dict):
        for key, child in value.items():
            key_text = str(key)
            output.append((path, key_text, child))
            output.extend(_iter_items(child, (*path, key_text)))
    elif isinstance(value, list):
        for child in value:
            output.extend(_iter_items(child, path))
    return output


def _payload_text(*payloads: dict[str, Any]) -> str:
    return " ".join(json.dumps(payload, sort_keys=True).lower() for payload in payloads if payload)


def _protected_requested(*payloads: dict[str, Any]) -> bool:
    for payload in payloads:
        if not isinstance(payload, dict):
            continue
        for path, raw_key, value in _iter_items(payload):
            path_lower = {part.lower() for part in path}
            if "protected_actions_blocked" in path_lower or "approval_required" in path_lower:
                continue
            key = raw_key.lower()
            if isinstance(value, dict) and key == "protected_action_flags":
                if any(bool(flag_value) for flag_value in value.values()):
                    return True
            if value is True and (key in PROTECTED_REQUEST_KEYS or key in REQUESTED_PROTECTED_KEYS):
                return True
    return False


def _collect_safety(*payloads: dict[str, Any]) -> dict[str, bool]:
    safety = _base_safety()
    if SANDBOX_1312_MARKER in _payload_text(*payloads):
        safety["sandbox_1312_blocker"] = True
    if _protected_requested(*payloads):
        safety["protected_action_requested"] = True
    return safety


def evaluate_apply_approval_gate(
    selected_queue_item: dict[str, Any] | None = None,
    approval_request: dict[str, Any] | None = None,
) -> dict[str, Any]:
    queue_item = _as_dict(selected_queue_item)
    request = _as_dict(approval_request)
    selected_queue_action = str(queue_item.get("action_id", "none"))
    requested_action = str(
        _first_present(
            request.get("requested_action"),
            request.get("action_id"),
            request.get("selected_queue_action"),
            default="none",
        )
    )
    approved_by = str(_first_present(request.get("approved_by"), default=""))
    approval_token_present = bool(request.get("approval_token_present") is True)
    queue_allowed_paths = _normalize_paths(queue_item.get("allowed_paths", []))
    requested_write_paths = _normalize_paths(
        _first_present(
            request.get("requested_write_paths"),
            request.get("write_paths"),
            request.get("allowed_paths"),
            default=[],
        )
    )
    validators = [str(validator) for validator in queue_item.get("validators", []) if str(validator)]
    safety = _collect_safety(queue_item, request)
    allowed_paths_match = bool(
        requested_write_paths
        and queue_allowed_paths
        and set(requested_write_paths).issubset(set(queue_allowed_paths))
    )
    validators_present = bool(validators)

    rejection_reasons: list[str] = []
    review_reasons: list[str] = []
    if safety["sandbox_1312_blocker"]:
        review_reasons.append("sandbox_1312_runner_blocker")
    if not approval_token_present:
        review_reasons.append("approval_token_missing")
    if approved_by != AUTHORIZED_APPROVER:
        review_reasons.append("authorized_approver_missing")
    if requested_action != selected_queue_action:
        rejection_reasons.append("requested_action_mismatch")
    if not allowed_paths_match:
        rejection_reasons.append("requested_paths_outside_queue_allowed_paths")
    if not validators_present:
        rejection_reasons.append("validators_missing")
    if safety["protected_action_requested"]:
        rejection_reasons.append("protected_action_requested")

    local_allowlisted_apply_allowed = bool(
        approved_by == AUTHORIZED_APPROVER
        and approval_token_present
        and requested_action == selected_queue_action
        and allowed_paths_match
        and validators_present
        and not safety["protected_action_requested"]
    )

    if rejection_reasons:
        approval_status = "rejected"
    elif local_allowlisted_apply_allowed:
        approval_status = "approved"
    else:
        approval_status = "review_required"

    if approval_status == "approved":
        next_safe_action = "Anthony approval is present; local allowlisted APPLY may be prepared, but v0 still cannot apply without human."
    elif safety["sandbox_1312_blocker"]:
        next_safe_action = "Treat sandbox 1312 as a local runner blocker, not a code failure."
    elif approval_status == "review_required":
        next_safe_action = "Stop until Anthony provides explicit approval for this selected queue action."
    else:
        next_safe_action = "Stop and repair the rejected approval request before any local APPLY."

    return {
        "schema": SCHEMA,
        "approval_status": approval_status,
        "requested_action": requested_action,
        "selected_queue_action": selected_queue_action,
        "approved_by": approved_by or "none",
        "approval_token_present": approval_token_present,
        "allowed_paths_match": allowed_paths_match,
        "validators_present": validators_present,
        "protected_actions_blocked": PROTECTED_ACTIONS_BLOCKED.copy(),
        "local_allowlisted_apply_allowed": local_allowlisted_apply_allowed,
        "can_apply_without_human": False,
        "rejection_reasons": rejection_reasons or review_reasons,
        "next_safe_action": next_safe_action,
        "safety": safety,
    }


build_self_build_apply_approval_gate = evaluate_apply_approval_gate
evaluate_self_build_apply_approval_gate = evaluate_apply_approval_gate


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate AIOS self-build local APPLY approval.")
    parser.add_argument("--queue-item", default="{}")
    parser.add_argument("--approval-request", default="{}")
    return parser


def main(argv: list[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    try:
        queue_item = json.loads(args.queue_item)
        request = json.loads(args.approval_request)
    except json.JSONDecodeError:
        queue_item = {}
        request = {}
    print(json.dumps(evaluate_apply_approval_gate(queue_item, request), indent=2, sort_keys=False))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
