...
FORBIDDEN_ACTIONS = [
    ...
    "Reports commits unless approved",
    "control/review_bridge commits unless approved",
    "broker",
    "live trading",
    "credentials",
    "secrets",
...
def build_bored_work_queue(report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _bored_task(
            "PKT-AIOS-BORED-REPO-HYGIENE-DOCS-ONE",
            "Clarify one repo hygiene doc acceptance point",
            "bored-repo-hygiene-docs",
            "No active packet; narrow repo hygiene docs reduce future operator confusion.",
            ["docs/orchestration/...", "docs/workflows/..."],
            ["python -m pytest -p no:cacheprovider tests/orchestration/test_aios_operator_checkpoint_dashboard.py -q"],
        ),
        _bored_task(
            "PKT-AIOS-BORED-FOREX-ROADMAP-ACCEPTANCE-ONE",
            "Clarify one forex roadmap acceptance criterion",
            "bored-forex-roadmap-acceptance",
            "No active packet; forex roadmap acceptance criteria keep the proof product moving safely.",
            ["docs/trading_lab/...", "tests/orchestration/..."],
            ["python -m pytest -p no:cacheprovider tests/orchestration/test_aios_forex_builder_roadmap.py -q"],
        ),
        _bored_task(
            "PKT-AIOS-BORED-DASHBOARD-SIMPLIFICATION-ONE",
            "Simplify one compact dashboard assertion",
            "bored-dashboard-simplification",
            "No active packet; compact dashboard hardening keeps the operator checkpoint readable.",
        ...
        _bored_task(
            "PKT-AIOS-BORED-RISK-GATE-TEST-ONE",
            "Harden one forex risk gate test",
            "bored-risk-gate-test",
            "No active packet; risk gate tests improve paper-only safety evidence.",
            ["tests/forex_engine/..."],
            ["python -m pytest -p no:cacheprovider tests/forex_engine/test_risk_contract.py -q"],
        ),
        _bored_task(
            "PKT-AIOS-BORED-DAILY-LOOP-PREP-ONE",
            "Prepare one daily contribution loop assertion",
            "bored-daily-loop-prep",
            "No active packet; daily loop assertions keep earned repo work legitimate.",
