...
param(
    ...
    [switch]$OutputJson,
    [string]$CommandDecision = "COMMAND_PREVIEW_READY_AWAITING_APPROVAL",
...
function Test-FullAutonomyWorkerLaunchGuardDirtyState {
    param([object]$State)
    $allowedExact = @(
...
function Invoke-PythonJsonLogic {
    ...
    if (-not (Test-Path -LiteralPath $LogicPath -PathType Leaf)) {
        throw "Python full-autonomy worker launch guard logic module missing: $LogicPath"
    }
    ...
    if (-not $process.WaitForExit($TimeoutSecondsValue * 1000)) {
        $process.Kill()
        throw "Python full-autonomy worker launch guard logic timed out."
    }
    $rawText = $stdoutTask.Result.Trim()
    $errorText = $stderrTask.Result.Trim()
    if ([string]::IsNullOrWhiteSpace($rawText)) {
        throw "Python full-autonomy worker launch guard logic returned no JSON. $errorText"
    }
...
function Write-ConsoleReport {
    param([object]$Result)
    Write-Host "AIOS Full Autonomy Worker Launch Guard"
    ...
    Write-Host ""
    Write-Host "GUARD DECISION"
    Write-Host "command_decision: $($Result.input_command_summary.command_decision)"
    Write-Host "preflight_decision: $($Result.input_command_summary.preflight_decision)"
    Write-Host "guard_decision: $($Result.guard_decision)"
    Write-Host "launch_allowed_for_future_packet: $($Result.launch_allowed_for_future_packet)"
    Write-Host "launch_executed: $($Result.launch_executed)"
    Write-Host ""
    Write-Host "LANES AND WORKERS"
    Write-Host "allowed_lanes: $(@($Result.lane_state.allowed_lanes) -join ', ')"
    Write-Host "requested_worker_count: $($Result.worker_count_state.requested_worker_count)"
    Write-Host "max_parallel_workers: $($Result.worker_count_state.max_parallel_workers)"
    Write-Host ""
    Write-Host "SAFETY"
...
$logicPath = Join-Path $PSScriptRoot "aios_full_autonomy_worker_launch_guard.py"
...
$dirtyAllowedForValidation = Test-FullAutonomyWorkerLaunchGuardDirtyState -State $beforeState
...
$repoState = [ordered]@{
    ...
    dirty_allowed_for_full_autonomy_worker_launch_guard_validation = [bool]$dirtyAllowedForValidation
