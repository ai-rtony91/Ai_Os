...
param(
    ...
    [string]$ActivationGateStatus = "PASS",
    [string]$WorkerPostureBridgeStatus = "PASS",
    [string]$LockCollisionStatus = "CLEAR",
...
function Test-FullAutonomyWorkerLaunchPreflightGateDirtyState {
    param([object]$State)
    $allowedExact = @(
        "automation/orchestration/self_development/aios_full_autonomy_worker_launch_preflight_gate.py",
        "automation/orchestration/self_development/Test-AiOsFullAutonomyWorkerLaunchPreflightGate.DRY_RUN.ps1",
        "schemas/aios/orchestration/AIOS_FULL_AUTONOMY_WORKER_LAUNCH_PREFLIGHT_GATE_RESULT.v1.schema.json",
        "schemas/aios/orchestration/ORCHESTRATION_SCHEMA_INDEX.json",
        "tests/orchestration/test_aios_full_autonomy_worker_launch_preflight_gate.py",
        "tests/orchestration/test_aios_full_autonomy_worker_launch_preflight_gate_runner.py",
...
function Invoke-PythonJsonLogic {
    ...
    if (-not (Test-Path -LiteralPath $LogicPath -PathType Leaf)) {
        throw "Python full-autonomy worker launch preflight gate logic module missing: $LogicPath"
    }
    ...
    if (-not $process.WaitForExit($TimeoutSecondsValue * 1000)) {
        $process.Kill()
        throw "Python full-autonomy worker launch preflight gate logic timed out."
    }
    $rawText = $stdoutTask.Result.Trim()
    $errorText = $stderrTask.Result.Trim()
    if ([string]::IsNullOrWhiteSpace($rawText)) {
        throw "Python full-autonomy worker launch preflight gate logic returned no JSON. $errorText"
    }
...
function Write-ConsoleReport {
    param([object]$Result)
    Write-Host "AIOS Full Autonomy Worker Launch Preflight Gate"
    ...
    Write-Host "requested_worker_posture: $($Result.requested_worker_posture)"
    Write-Host "decision: $($Result.preflight_decision.decision)"
    Write-Host "worker_launch_allowed_for_future_step: $($Result.worker_launch_allowed_for_future_step)"
    Write-Host "worker_launch_executed: $($Result.worker_launch_executed)"
    Write-Host ""
    ...
    Write-Host "policy: $($Result.human_wake_policy.policy)"
    Write-Host ""
...
$logicPath = Join-Path $PSScriptRoot "aios_full_autonomy_worker_launch_preflight_gate.py"
...
$dirtyAllowedForValidation = Test-FullAutonomyWorkerLaunchPreflightGateDirtyState -State $beforeState
...
$repoState = [ordered]@{
    ...
    dirty_allowed_for_full_autonomy_worker_launch_preflight_gate_validation = [bool]$dirtyAllowedForValidation
...
$payload = [ordered]@{
    ...
    worker_posture_bridge_status = $WorkerPostureBridgeStatus
    lock_collision_status = $LockCollisionStatus
