...
Write-Host 'Task name: AI_OS Stage 24A-24D Morning Brief Preview Generator Dry Run'
Write-Host "Mode: $mode"
Write-Host "Repo root: $RepoRoot"
Write-Host 'Safety: Console-output only. No files are created, edited, moved, renamed, deleted, staged, committed, pushed, launched, opened, settings-changed, broker-routed, webhook-fired, credential-accessed, secrets-accessed, telemetry-written, Morning-Brief-written, or traded.'
Write-Host ''

if (-not (Test-Path -LiteralPath $RepoRoot -PathType Container)) {
    Write-Host 'Conceptual Morning Brief preview state: FAIL_BLOCKED'
    Write-Host 'PASS/WARN/FAIL summary: FAIL'
    Write-Host "Repo root missing: $RepoRoot"
    Write-Host ('DRY_RUN COMPLETE {0} NO MORNING BRIEF PREVIEW ACTIONS APPLIED.' -f [char]0x2014)
    exit 1
}
...
Test-RequiredFile -Label 'Morning Brief preview generator' -RelativePath 'automation\startup\Preview-AiOsMorningBrief.DRY_RUN.ps1'
...
Test-RequiredFile -Label 'Morning Brief generator boundary' -RelativePath 'docs\AI_OS\morning_brief\AIOS_MORNING_BRIEF_GENERATOR_BOUNDARY_DRAFT.md'
Test-RequiredFile -Label 'telemetry preview validator' -RelativePath 'automation\status\Test-AiOsTelemetryPreviewContract.DRY_RUN.ps1'
Test-RequiredFile -Label 'daily report preview validator' -RelativePath 'automation\status\Test-AiOsDailyReportPreviewGenerator.DRY_RUN.ps1'

$previewScriptPath = Join-Path $script:ResolvedRepoRoot 'automation\startup\Preview-AiOsMorningBrief.DRY_RUN.ps1'
$previewScriptText = ''
if (Test-Path -LiteralPath $previewScriptPath -PathType Leaf) {
    $previewScriptText = Get-Content -LiteralPath $previewScriptPath -Raw
}
...
$requiredPhrases = @(
    'DRY_RUN COMPLETE',
    'NO MORNING BRIEF FILES WRITTEN',
    'proposed target file',
    'blocked fields',
    'validator to run after future write'
)

foreach ($phrase in $requiredPhrases) {
    Test-Text -Label $phrase -Text $previewScriptText -Expected $phrase
}
...
if ($failures.Count -gt 0) {
    $morningBriefPreviewState = 'FAIL_BLOCKED'
}
elseif ($warnings.Count -gt 0) {
    $morningBriefPreviewState = 'WARN_REVIEW_REQUIRED'
}
else {
    $morningBriefPreviewState = 'MORNING_BRIEF_PREVIEW_READY_FOR_REVIEW'
}

Write-Host "Conceptual Morning Brief preview state: $morningBriefPreviewState"
Write-Host ''
if ($failures.Count -gt 0) {
    Write-Host 'PASS/WARN/FAIL summary: FAIL'
