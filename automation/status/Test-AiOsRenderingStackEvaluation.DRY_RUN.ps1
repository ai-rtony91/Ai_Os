...
Write-Host "Task name: AI_OS Stage 40A-40F Rendering Stack Evaluation Dry Run"
...
try {
    $stage40Files = @(
        ...
        "Reports\health\STAGE40A_40F_RENDERING_STACK_EVALUATION_README.txt"
    )

    Write-Host "File checks:"
    Test-RequiredFile "rendering stack evaluation draft" $stage40Files[0] | Out-Null
    Test-RequiredFile "frontend runtime architecture draft" $stage40Files[1] | Out-Null
    Test-RequiredFile "UI performance requirements draft" $stage40Files[2] | Out-Null
    Test-RequiredFile "rendering stack evaluation validator" $stage40Files[3] | Out-Null
    Test-RequiredFile "Stage 40 health README" $stage40Files[4] | Out-Null
    ...
    foreach ($phrase in $phrases) {
        Test-RequiredPhrase $phrase $stage40Files | Out-Null
    }
...
Write-Host "DRY_RUN COMPLETE - NO RENDERING STACK EVALUATION ACTIONS APPLIED."
