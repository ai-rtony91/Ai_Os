...
Write-Host 'Task name: AI_OS Stage 22A-22D Daily Report Preview Generator Dry Run'
...
if (-not (Test-Path -LiteralPath $RepoRoot -PathType Container)) {
    ...
    Write-Host ('DRY_RUN COMPLETE {0} NO DAILY REPORT PREVIEW ACTIONS APPLIED.' -f [char]0x2014)
    exit 1
}
...
Write-Host ''
Write-Host 'Preview script phrase checks:'
$requiredPhrases = @(
    'DRY_RUN COMPLETE',
