...
Write-Host 'Task name: AI_OS Stage 21A-21D Report Writer Preview Contract Dry Run'
...
if (-not (Test-Path -LiteralPath $RepoRoot -PathType Container)) {
    Write-Host 'Conceptual report-writer preview state: FAIL_BLOCKED'
    Write-Host 'PASS/WARN/FAIL summary: FAIL'
    Write-Host "Repo root missing: $RepoRoot"
    Write-Host ('DRY_RUN COMPLETE {0} NO REPORT-WRITER ACTIONS APPLIED.' -f [char]0x2014)
    exit 1
}
...
if ($failures.Count -gt 0) {
    $reportWriterPreviewState = 'FAIL_BLOCKED'
}
elseif ($warnings.Count -gt 0) {
    $reportWriterPreviewState = 'WARN_REVIEW_REQUIRED'
}
else {
    $reportWriterPreviewState = 'REPORT_PREVIEW_READY_FOR_REVIEW'
}

Write-Host "Conceptual report-writer preview state: $reportWriterPreviewState"
Write-Host ''
if ($failures.Count -gt 0) {
    Write-Host 'PASS/WARN/FAIL summary: FAIL'
