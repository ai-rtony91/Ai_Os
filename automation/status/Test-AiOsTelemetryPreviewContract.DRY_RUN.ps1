...
Write-Host 'Task name: AI_OS Stage 23A-23D Telemetry Preview Contract Dry Run'
Write-Host "Mode: $mode"
Write-Host "Repo root: $RepoRoot"
Write-Host 'Safety: Console-output only. No files are created, edited, moved, renamed, deleted, staged, committed, pushed, launched, opened, settings-changed, broker-routed, webhook-fired, credential-accessed, secrets-accessed, telemetry-written, report-written, or traded.'
Write-Host ''

if (-not (Test-Path -LiteralPath $RepoRoot -PathType Container)) {
    Write-Host 'Conceptual telemetry preview state: FAIL_BLOCKED'
    Write-Host 'PASS/WARN/FAIL summary: FAIL'
    Write-Host "Repo root missing: $RepoRoot"
    Write-Host ('DRY_RUN COMPLETE {0} NO TELEMETRY PREVIEW ACTIONS APPLIED.' -f [char]0x2014)
    exit 1
}
...
Test-RequiredFile -Label 'telemetry preview helper' -RelativePath 'automation\telemetry\Preview-AiOsTelemetry.DRY_RUN.ps1'
...
$filesToScan = @(
    ...
    'automation\telemetry\Preview-AiOsTelemetry.DRY_RUN.ps1',
...
if ($failures.Count -gt 0) {
    $telemetryPreviewState = 'FAIL_BLOCKED'
}
elseif ($warnings.Count -gt 0) {
    $telemetryPreviewState = 'WARN_REVIEW_REQUIRED'
}
else {
    $telemetryPreviewState = 'TELEMETRY_PREVIEW_READY_FOR_REVIEW'
}

Write-Host "Conceptual telemetry preview state: $telemetryPreviewState"
Write-Host ''
if ($failures.Count -gt 0) {
    Write-Host 'PASS/WARN/FAIL summary: FAIL'
