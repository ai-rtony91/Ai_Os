"""Build a review-only diff for Worker Branch and Lane Rules authority."""
...
REPORT_TYPE = "operator_relief_worker_branch_lane_rules_decision_diff_v1"
SOURCE_DECISION_PACKET = Path(
    "Reports/operator_relief/decision_packets/canonical_decision_packet_01_worker_branch_and_lane_rules.json"
)
REPORT_PATH = Path("Reports/operator_relief/decision_packets/worker_branch_lane_rules_decision_diff.json")
CANONICAL_CANDIDATE = Path("docs/workflows/WORKER_BRANCH_AND_LANE_RULES.md")
DUPLICATE_CANDIDATE = Path("docs/AI_OS/operator/AIOS_WORKER_BRANCH_AND_LANE_RULES.md")
DEFAULT_DEPENDENCIES = (Path("docs/audits/phase-5c-narrow-merge-plan.md"),)
            ...
            findings.append(
                {
                    ...
                    "reason": "Dependency contains worker-lane, canonical, promotion, validation, or user-decision guidance.",
                }
            )
    return findings
...
def _reasons(
    ...
    if dependency_authority:
        reasons.append("Audit dependency contains unique authority or decision guidance that should not be lost.")
    if not duplicate_only and not conflicts and not dependency_authority:
...
def _risks(
    ...
    if duplicate_only:
        risks.append("Duplicate candidate may contain unique rules that need extraction before any later deprecation review.")
    if conflicts:
        risks.append("Conflicting shared sections may hide rule drift between active and historical workflow text.")
    if dependency_authority:
        risks.append("Audit dependency includes unique authority or user-decision guidance that must be reviewed first.")
    return risks


def build_decision_diff(repo_root: Path) -> DecisionDiffResult:
    root = repo_root.resolve()
    source_packet_path = root / SOURCE_DECISION_PACKET
    canonical_path = root / CANONICAL_CANDIDATE
    duplicate_path = root / DUPLICATE_CANDIDATE

    source_packet = json.loads(_read_text(source_packet_path) or "{}")
    dependency_paths = [
        Path(path)
        for path in source_packet.get("dependencies", [])
        if isinstance(path, str)
    ] or list(DEFAULT_DEPENDENCIES)

    canonical_text = _read_text(canonical_path)
    duplicate_text = _read_text(duplicate_path)
    ...
    decision = _recommended_decision(duplicate_only, conflicts, dependency_authority)
    ...
    return DecisionDiffResult(
        ...
        risks=_risks(duplicate_only, conflicts, dependency_authority),
        safe_to_generate_apply_packet_later=decision == KEEP_WORKFLOWS_AS_CANONICAL,
        reasons=_reasons(identical, canonical_only, duplicate_only, conflicts, dependency_authority),
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build Worker Branch and Lane Rules decision diff.")
