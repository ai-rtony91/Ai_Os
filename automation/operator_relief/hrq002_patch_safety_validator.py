"""Validate the DRY_RUN-only HRQ-002 draft patch."""
...
REPORT_TYPE = "operator_relief_hrq002_patch_safety_validator_v1"
PATCH_ROOT = Path("Reports/operator_relief/hrq002_apply_draft_patch")
DIFF_PATH = PATCH_ROOT / "hrq002_apply_draft_patch.diff"
PATCH_JSON_PATH = PATCH_ROOT / "hrq002_apply_draft_patch.json"
OUTPUT_PATH = PATCH_ROOT / "hrq002_patch_safety_validation.json"
CANONICAL_TARGET = "docs/workflows/PARALLEL_CODEX_WORKFLOW.md"
DUPLICATE_SOURCE = "docs/AI_OS/operator/AIOS_PARALLEL_CODEX_WORKFLOW.md"
APPROVED_SECTIONS = [
    "Worker Lanes",
    "Start DRY_RUN Crew",
...
class HRQ002PatchSafetyValidationResult:
    report_type: str
    generated_at: str
    executable: bool
    source_diff: str
...
def _included_section_names(patch_json: dict[str, Any]) -> list[str]:
    ...
    for section in sections:
        if isinstance(section, dict):
            names.append(str(section.get("source_section", "")))
    return names
...
def _validate_patch(diff_text: str, patch_json: dict[str, Any]) -> list[str]:
    ...
    if expected_from not in headers or expected_to not in headers:
        errors.append("patch must target only docs/workflows/PARALLEL_CODEX_WORKFLOW.md")
    if any((DUPLICATE_SOURCE in header) for header in headers):
    ...
    for section in APPROVED_SECTIONS:
        if f"### {section}" not in added_text:
            errors.append(f"approved section missing from draft diff: {section}")
    included_names = _included_section_names(patch_json)
    if included_names != APPROVED_SECTIONS:
        errors.append("sections_included must be exactly the six approved HRQ-002 sections")
    ...
    rollback_plan = patch_json.get("rollback_plan")
    if not isinstance(rollback_plan, list) or not rollback_plan:
        errors.append("rollback_plan must exist and be non-empty")
    validation_commands = patch_json.get("validation_commands")
    if not isinstance(validation_commands, list) or not validation_commands:
        errors.append("validation_commands must exist and be non-empty")
    return errors


def build_validation(repo_root: Path) -> HRQ002PatchSafetyValidationResult:
    ...
    return HRQ002PatchSafetyValidationResult(
        ...
        recommended_next_action=(
            ...
            if accepted
            else "Fix rejected HRQ-002 draft patch evidence before human review or APPLY consideration."
        ),
    )
...
def _output_path(repo_root: Path) -> Path:
    ...
    if not (output.parent == allowed_root and allowed_root in output.parents):
        raise ValueError("HRQ-002 patch safety validation must be written under Reports/operator_relief/hrq002_apply_draft_patch/.")
    return output


def write_validation(result: HRQ002PatchSafetyValidationResult, repo_root: Path) -> Path:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate DRY_RUN-only HRQ-002 draft patch.")
    ...
    if args.write_report:
        repo_root = Path.cwd()
        payload["written_file"] = _repo_relative_path(write_validation(result, repo_root), repo_root)
