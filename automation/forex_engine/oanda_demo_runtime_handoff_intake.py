...
BROKER_ID = "OANDA"
BROKER_REFERENCE = "OANDA_DEMO_RUNTIME_HANDOFF_INTAKE_REFERENCE_ONLY"
INTAKE_READY = "OANDA_DEMO_RUNTIME_HANDOFF_INTAKE_READY"
INTAKE_BLOCKED = "OANDA_DEMO_RUNTIME_HANDOFF_INTAKE_BLOCKED"
...
class OandaDemoRuntimeHandoffIntake:
    broker_id: str = BROKER_ID
    account_mode: str = "PRACTICE_DEMO"
    environment: str = "OANDA_PRACTICE_DEMO"
    endpoint_classification: str = "OANDA_PRACTICE_DEMO"
    intake_scope: str = INTAKE_SCOPE
    intake_mode: str = INTAKE_MODE
    metadata_intake_authorized: bool = True
    ...
    external_operator_controlled_runtime_confirmed: bool = True
...
def build_runtime_handoff_intake_contract() -> dict[str, Any]:
    contract = {
        "schema": "AIOS_OANDA_DEMO_RUNTIME_HANDOFF_INTAKE_CONTRACT.v1",
        "broker_id": BROKER_ID,
        "broker_reference": BROKER_REFERENCE,
        "mode": INTAKE_MODE,
        "intake_scope_required": INTAKE_SCOPE,
        "required_intake_fields": list(REQUIRED_INTAKE_FIELDS),
        ...
        "metadata_intake_authorization_required": True,
        "runtime_boundary_required": True,
        "external_operator_controlled_runtime_required": True,
    ...
    assert_no_oanda_demo_runtime_handoff_intake_side_effects(contract)
    return contract


def build_intake_metadata_acceptance_rules() -> dict[str, Any]:
    rules = {
        "schema": "AIOS_OANDA_DEMO_RUNTIME_HANDOFF_INTAKE_ACCEPTANCE_RULES.v1",
        "broker_id": BROKER_ID,
        "broker_reference": BROKER_REFERENCE,
        "accepted_metadata_only": True,
        "accepted_runtime_reference_value": False,
        "accepted_account_identifier_value": False,
        "accepted_auth_value": False,
        "accepted_account_modes": sorted(SUPPORTED_ACCOUNT_MODES),
        "accepted_environments": sorted(SUPPORTED_ENVIRONMENTS),
        "accepted_endpoint_classifications": sorted(SUPPORTED_ENDPOINT_CLASSIFICATIONS),
        "accepted_reference_format": AUTH_REFERENCE_FORMAT,
        "accepted_auth_material_location": AUTH_MATERIAL_LOCATION,
        "runtime_boundary_confirmation_required": True,
        "repo_storage_absence_confirmation_required": True,
        "audit_acknowledgement_required": True,
        **_blocked_capabilities(),
    }
    assert_no_oanda_demo_runtime_handoff_intake_side_effects(rules)
    return rules


def build_intake_metadata_rejection_rules() -> dict[str, Any]:
    rules = {
        "schema": "AIOS_OANDA_DEMO_RUNTIME_HANDOFF_INTAKE_REJECTION_RULES.v1",
        "broker_id": BROKER_ID,
        "broker_reference": BROKER_REFERENCE,
        "reject_account_identifiers": True,
        "reject_auth_values": True,
        "reject_credential_like_values": True,
        "reject_live_endpoint_references": True,
        "reject_live_account_references": True,
        "reject_malformed_runtime_references": True,
        "reject_unauthorized_intake_attempts": True,
        "forbidden_intake_field_names": sorted(FORBIDDEN_INTAKE_FIELD_NAMES),
        "fail_closed_default": True,
        **_blocked_capabilities(),
    }
    assert_no_oanda_demo_runtime_handoff_intake_side_effects(rules)
...
def build_runtime_handoff_intake_evidence_schema() -> dict[str, Any]:
    evidence_schema = {
        "schema": "AIOS_OANDA_DEMO_RUNTIME_HANDOFF_INTAKE_EVIDENCE_SCHEMA.v1",
        "broker_id": BROKER_ID,
        "broker_reference": BROKER_REFERENCE,
    ...
    assert_no_oanda_demo_runtime_handoff_intake_side_effects(evidence_schema)
    return evidence_schema


def build_oanda_demo_runtime_handoff_intake_contract_set() -> dict[str, Any]:
    contract_set = {
        "schema": "AIOS_OANDA_DEMO_RUNTIME_HANDOFF_INTAKE_CONTRACT_SET.v1",
        "broker_id": BROKER_ID,
        "broker_reference": BROKER_REFERENCE,
        "runtime_handoff_intake_contract": build_runtime_handoff_intake_contract(),
        "metadata_acceptance_rules": build_intake_metadata_acceptance_rules(),
        "metadata_rejection_rules": build_intake_metadata_rejection_rules(),
        "sanitized_intake_evidence_schema": build_runtime_handoff_intake_evidence_schema(),
        "contracts_ready_for_future_runtime_handoff_intake": True,
        **_blocked_capabilities(),
    }
    assert_no_oanda_demo_runtime_handoff_intake_side_effects(contract_set)
    return contract_set


def build_example_oanda_demo_runtime_handoff_intake() -> dict[str, Any]:
    return asdict(OandaDemoRuntimeHandoffIntake())


def evaluate_oanda_demo_runtime_handoff_intake(
    intake: OandaDemoRuntimeHandoffIntake | dict[str, Any] | None = None,
) -> dict[str, Any]:
    payload = _intake_payload(intake)
    ...
    status = INTAKE_BLOCKED if blockers else INTAKE_READY
    classification = "FAIL_CLOSED" if blockers else INTAKE_READY
    result = {
        "schema": "AIOS_OANDA_DEMO_RUNTIME_HANDOFF_INTAKE_RESULT.v1",
        "broker_id": BROKER_ID,
        "broker_reference": BROKER_REFERENCE,
        "status": status,
        "classification": classification,
        "runtime_handoff_intake_ready": not blockers,
        "metadata_accepted": not blockers,
        "runtime_boundary_enforced": not blockers,
        "metadata_intake_authorized": payload.get("metadata_intake_authorized") is True,
        ...
        "external_operator_controlled_runtime_confirmed": (
            payload.get("external_operator_controlled_runtime_confirmed") is True
        ),
        ...
        "blockers": blockers,
        "contract_set": build_oanda_demo_runtime_handoff_intake_contract_set(),
        "evidence_schema": build_runtime_handoff_intake_evidence_schema(),
        "audit_event": _build_intake_audit_event(
            status=status,
            classification=classification,
            blockers=blockers,
            forbidden_fields=forbidden_fields,
    ...
    assert_no_oanda_demo_runtime_handoff_intake_side_effects(result)
    return result


def summarize_oanda_demo_runtime_handoff_intake(result: dict[str, Any] | None = None) -> dict[str, Any]:
    payload = dict(result or evaluate_oanda_demo_runtime_handoff_intake())
    summary = {
        "schema": "AIOS_OANDA_DEMO_RUNTIME_HANDOFF_INTAKE_SUMMARY.v1",
        "broker_id": BROKER_ID,
        "broker_reference": BROKER_REFERENCE,
        "status": str(payload.get("status") or INTAKE_BLOCKED),
        "classification": classify_oanda_demo_runtime_handoff_intake(payload),
        "runtime_handoff_intake_ready": payload.get("runtime_handoff_intake_ready") is True,
        "metadata_accepted": payload.get("metadata_accepted") is True,
    ...
    assert_no_oanda_demo_runtime_handoff_intake_side_effects(summary)
    return summary


def classify_oanda_demo_runtime_handoff_intake(result: dict[str, Any] | None = None) -> str:
    payload = dict(result or evaluate_oanda_demo_runtime_handoff_intake())
    if list(payload.get("blockers") or []):
        return "FAIL_CLOSED"
    if payload.get("runtime_handoff_intake_ready") is True:
        return INTAKE_READY
    return "FAIL_CLOSED"


def assert_no_oanda_demo_runtime_handoff_intake_side_effects(payload: dict[str, Any]) -> bool:
    if _has_unsafe_capability(payload):
        raise ValueError("OANDA demo runtime handoff intake must not enable unsafe side effects")
...
def _intake_blockers(
    ...
    *,
    ...
    if payload.get("intake_scope") != INTAKE_SCOPE:
        blockers.append(f"intake_scope_must_equal:{INTAKE_SCOPE}")
    if payload.get("intake_mode") != INTAKE_MODE:
        blockers.append(f"intake_mode_must_equal:{INTAKE_MODE}")
    if payload.get("metadata_intake_authorized") is not True:
        blockers.append("metadata_intake_authorization_required")
    ...
    if payload.get("external_operator_controlled_runtime_confirmed") is not True:
        blockers.append("external_operator_controlled_runtime_confirmation_required")
...
def _build_intake_audit_event(
    *,
    ...
    event = {
        "schema": "AIOS_OANDA_DEMO_RUNTIME_HANDOFF_INTAKE_AUDIT_EVENT.v1",
        "event": "oanda_demo_runtime_handoff_intake_validated",
        "broker_id": BROKER_ID,
        "broker_reference": BROKER_REFERENCE,
        "status": status,
        "classification": classification,
    ...
    assert_no_oanda_demo_runtime_handoff_intake_side_effects(event)
    return event
...
def _next_safe_action(blockers: list[str]) -> str:
    if blockers:
        return (
            "Keep runtime handoff intake fail-closed until sanitized metadata proves "
            "auth values and account identifiers remain outside repo and chat."
        )
    return (
        "Runtime handoff intake is valid for future protected probe review only; no "
        "broker connection, authentication, account access, or order routing is authorized."
    )
...
def _intake_payload(intake: OandaDemoRuntimeHandoffIntake | dict[str, Any] | None) -> dict[str, Any]:
    if intake is None:
        return {}
    if isinstance(intake, OandaDemoRuntimeHandoffIntake):
...
def _forbidden_field_paths(value: Any, prefix: str = "") -> list[str]:
    ...
    if isinstance(value, dict):
        for key, nested in value.items():
            ...
            if normalized in FORBIDDEN_INTAKE_FIELD_NAMES:
                paths.append(path)
