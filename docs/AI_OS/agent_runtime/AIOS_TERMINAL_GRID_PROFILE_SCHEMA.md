...
| --- | --- | --- | --- |
| `schema_version` | string | Yes | Schema version for this planning contract. Use semantic version text such as `1.0.0`. |
...
| `profile_type` | string | Yes | Must describe this as a terminal grid layout profile. |
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `target_width_px` | integer | Yes | Planned display width in pixels. |
| `target_height_px` | integer | Yes | Planned display height in pixels. |
...
| `reserved_zones` | array | Yes | Optional zones reserved for future dashboards, logs, or command panels. Use an empty array when none are reserved. |
...
- Empty grid cells are allowed only when their purpose is documented in `reserved_zones`.

### `reserved_zones` array item

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `zone_id` | string | Yes | Unique identifier for the reserved zone. |
| `purpose` | string | Yes | Operator-readable reason for reserving the zone. |
| `enabled` | boolean | Yes | Must be `false` while the profile remains schema-only. |
...
4. Confirm every pane has `startup_command: null`.
5. Confirm pane IDs are unique, pane positions are in bounds, and pane rectangles do not overlap.
6. Confirm each pane blocks `live_trading`, `broker_connection`, `oanda_connection`, `startup_persistence`, and `scheduled_task_creation`.
7. Confirm the profile contains no credential values, API keys, secrets, broker endpoints, or executable commands.
8. Run `git diff --check` after changes.
