...
`one_action_execution_result_collector` is produced by `AIOS_SELF_BUILD_ONE_ACTION_EXECUTION_RESULT_COLLECTOR.v1`. It consumes the local APPLY executor and apply result verifier outputs after the executor stage. In the current DRY_RUN flow, no command has run, so it reports `collector_status: blocked`, `command_executed: false`, and `result_safe_to_package: false`. After a separately approved injected local APPLY execution and passed verifier evidence, it can report `collector_status: collected` and `result_safe_to_package: true`, but it still does not write files, write reports, stage, commit, push, merge, or execute commands.
